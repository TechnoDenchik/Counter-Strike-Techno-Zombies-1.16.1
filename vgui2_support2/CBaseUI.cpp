...
void CBaseUI::Initialize(CreateInterfaceFn* factories, int count) {
	...
	vgui2::filesystem()->AddSearchPath("cstrike", "GAME_FALLBACK");
...
void CBaseUI::Shutdown() {
	...
	Sys_UnloadModule(m_hVGui2Module);
	m_hVGui2Module = NULL;
	Sys_UnloadModule(m_hVGui1Module);
	m_hVGui1Module = NULL;
