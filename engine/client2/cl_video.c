...
void SCR_CheckStartupVids( void )
{
	...
	if( !FS_FileExists( "media/media/playlist2.json", false ))
		SCR_CreateStartupVids();

	afile = (char *)FS_LoadFile( "media/media/playlist2.json", NULL, false );
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		if( ++c > MAX_MOVIES - 1 )
		{
			MsgDev( D_WARN, "Host_StartMovies: max %i movies in playlist2.json\n", MAX_MOVIES );
			break;
		}
	}
