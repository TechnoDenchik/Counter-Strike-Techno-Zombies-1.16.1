...
qboolean Cmd_GetMapList( const char *s, char *completedname, int length )
{
	...
	/*t = FS_Search(va("orig/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;

	t = FS_Search(va("dm/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;

	t = FS_Search(va("tdm/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;

	t = FS_Search(va("gdm/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;

	t = FS_Search(va("zb2/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;

	t = FS_Search(va("zb3/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;

	t = FS_Search(va("zsh/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;

	t = FS_Search(va("zs/%s*.bsp", s), false, con_gamemaps->integer);
	if (!t) return false;*/
...
qboolean Cmd_GetMovieList( const char *s, char *completedname, int length )
{
	...
	t = FS_Search( va( "media/%s*.mp4", s ), true, false );
	if( !t ) return false;
...
qboolean Cmd_CheckMapsList_R( qboolean fRefresh, qboolean onlyingamedir )
{
	...
	/*if (FS_FileSize("orig.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}

	if (FS_FileSize("dm.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}

	if (FS_FileSize("tdm.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}

	if (FS_FileSize("gmd.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}

	if (FS_FileSize("zb2.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}

	if (FS_FileSize("zb3.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}

	if (FS_FileSize("zsh.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}

	if (FS_FileSize("zs.lst", onlyingamedir) > 0 && !fRefresh)
	{
		MsgDev(D_NOTE, "maps.lst found: %s\n", onlyingamedir ? "basedir" : "gamedir");
		return true; // exists
	}*/
	...
	/*t = FS_Search("orig/*.bsp", false, onlyingamedir);
	t = FS_Search( "dm/*.bsp", false, onlyingamedir);
	t = FS_Search( "tdm/*.bsp", false, onlyingamedir);
	t = FS_Search( "gdm/*.bsp", false, onlyingamedir);
	t = FS_Search( "zb2/*.bsp", false, onlyingamedir);
	t = FS_Search( "zb3/*.bsp", false, onlyingamedir);
	t = FS_Search( "zsh/*.bsp", false, onlyingamedir);
	t = FS_Search( "zs/*.bsp", false, onlyingamedir);*/
	...
	/*if (FS_WriteFile("dm.lst", buffer, Q_strlen(buffer)))
	{
		if (buffer) Mem_Free(buffer);
		return true;
	}

	if (FS_WriteFile("tdm.lst", buffer, Q_strlen(buffer)))
	{
		if (buffer) Mem_Free(buffer);
		return true;
	}

	if (FS_WriteFile("gdm.lst", buffer, Q_strlen(buffer)))
	{
		if (buffer) Mem_Free(buffer);
		return true;
	}

	if (FS_WriteFile("zb2.lst", buffer, Q_strlen(buffer)))
	{
		if (buffer) Mem_Free(buffer);
		return true;
	}

	if (FS_WriteFile("zb3.lst", buffer, Q_strlen(buffer)))
	{
		if (buffer) Mem_Free(buffer);
		return true;
	}

	if (FS_WriteFile("zsh.lst", buffer, Q_strlen(buffer)))
	{
		if (buffer) Mem_Free(buffer);
		return true;
	}

	if (FS_WriteFile("zs.lst", buffer, Q_strlen(buffer)))
	{
		if (buffer) Mem_Free(buffer);
		return true;
	}*/
