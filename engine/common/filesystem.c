...
static qboolean FS_WriteGameInfo( const char *filepath, gameinfo_t *GameInfo )
{
	...
	FS_Print( f, "/* Generated By FS_Print * TechnoSoftware Inc */\n\n\n" );
...
void FS_InitGameInfo( gameinfo_t *GameInfo, const char *gamedir )
{
	...
	Q_strncpy( GameInfo->basedir, "Counter-Strike-TZ", sizeof( GameInfo->basedir ));
	GameInfo->falldir[0] = 0;
	Q_strncpy( GameInfo->title, "Counter-Strike-TZ", sizeof( GameInfo->title ));
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		else if( !Q_stricmp( token, "type" ))
		{
			...
			if( !isGameInfo && !Q_stricmp( token, "singleplayer_only" ))
			{
				...
				if( !Q_stricmp( GameInfo->gamefolder, "Counter-Strike-TZ") )
		...
		{
			pfile = COM_ParseFile( pfile, token );
			GameInfo->max_edicts = bound( 600, Q_atoi( token ), 99999 );
		}
		// only for gameinfo
		else if( isGameInfo )
		{
			...
			{
				pfile = COM_ParseFile( pfile, token );
				GameInfo->max_tents = bound( 300, Q_atoi( token ), 9999 );
			}
			else if( !Q_stricmp( token, "max_beams" ))
			{
				pfile = COM_ParseFile( pfile, token );
				GameInfo->max_beams = bound( 64, Q_atoi( token ), 9999 );
			}
			else if( !Q_stricmp( token, "max_particles" ))
			{
				pfile = COM_ParseFile( pfile, token );
				GameInfo->max_particles = bound( 1024, Q_atoi( token ), 99999 );
			}
...
dll_user_t *FS_FindLibrary( const char *dllname, qboolean directpath )
{
	...
	if( !Q_strnicmp( dllname, "..\\Counter-Strike-TZ\\", 9 ) || !Q_strnicmp( dllname, "../Counter-Strike-TZ/", 9 ))
		start += 9;
...
wfile_t *W_Open( const char *filename, const char *mode )
{
	...
	const char	*comment = "Generated by TechnoEngine WadLib. ";
