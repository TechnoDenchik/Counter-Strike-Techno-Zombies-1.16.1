...
static qboolean FS_WriteGameInfo( const char *filepath, gameinfo_t *GameInfo )
{
	...
	FS_Print( f, "// Generated By TechnoSoftware\n\n\n" );
...
void FS_InitGameInfo( gameinfo_t *GameInfo, const char *gamedir )
{
	...
	Q_strncpy( GameInfo->title, "Cstrike T:Z", sizeof( GameInfo->title ));
	...
	GameInfo->max_edicts     = 9000; // default value if not specified
	GameInfo->max_tents      = 5000;
	GameInfo->max_beams      = 2048;
...
static qboolean FS_ParseGameInfo( const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_snprintf( filepath, sizeof( filepath ), "%s/cl_dlls/client.cst", gamedir );
	Q_snprintf( liblist, sizeof( liblist ), "%s/cl_dlls/gamesettings.json", gamedir );
	...
	if( host.rodir[0] )
	{
		...
		Q_snprintf( filepath_ro, sizeof( filepath_ro ), "%s/%s/client.cst", host.rodir, gamedir );
		Q_snprintf( liblist_ro, sizeof( liblist_ro ), "%s/%s/gamesettings.json", host.rodir, gamedir );
...
wfile_t *W_Open( const char *filename, const char *mode )
{
	...
	const char	*comment = "Generated By TechnoSoftware FileSystem. ";
