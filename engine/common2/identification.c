...
void ID_Init( void )
{
	...
	{
		CHAR szBuf[MAX_PATH];
		ID_GetKeyData( HKEY_CURRENT_USER, "Software\\TechnoEngine\\", "cst", szBuf, MAX_PATH );
...
#else
	{
		...
		if( home )
		{
			FILE *cfg = fopen( va( "%s/.config/.cst", home ), "r" );
			if( !cfg )
				cfg = fopen( va( "%s/.local/.cst", home ), "r" );
			if( !cfg )
				cfg = fopen( va( "%s/.cst", home ), "r" );
			if( cfg )
			{
	...
	if( !id )
	{
		const char *buf = (const char*) FS_LoadFile( ".cst", NULL, false );
		if( buf )
		{
...
#elif defined(_WIN32) && !defined(XASH_WINRT)
	{
		...
		ID_SetKeyData( HKEY_CURRENT_USER, "Software\\TechnoEngine\\", REG_SZ, "cst", Buf, Q_strlen(Buf) );
	}
#else
	{
#ifndef __HAIKU__
		...
		if( home )
		{
			FILE *cfg = fopen( va( "%s/.config/.cst", home ), "w" );
			if( !cfg )
				cfg = fopen( va( "%s/.local/.cst", home ), "w" );
			if( !cfg )
				cfg = fopen( va( "%s/.cst", home ), "w" );
			if( cfg )
			{
...
#endif
	FS_WriteFile( ".cst", va("%016llX", id^GAME_XOR_MASK), 16 );
#if 0
	Msg("MD5 id: %s\nRAW id:%016llX\n", id_md5, id );
#endif
}
