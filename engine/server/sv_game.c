...
void SV_WriteEntityPatch( const char *filename )
{
	...
	/*f = FS_Open(va("orig/%s.bsp", filename), "rb", false);
	if (!f) return;

	f = FS_Open(va("dm/%s.bsp", filename), "rb", false);
	if (!f) return;

	f = FS_Open(va("tdm/%s.bsp", filename), "rb", false);
	if (!f) return;

	f = FS_Open(va("gdm/%s.bsp", filename), "rb", false);
	if (!f) return;

	f = FS_Open(va("zb2/%s.bsp", filename), "rb", false);
	if (!f) return;

	f = FS_Open(va("zb3/%s.bsp", filename), "rb", false);
	if (!f) return;

	f = FS_Open(va("zsh/%s.bsp", filename), "rb", false);
	if (!f) return;

	f = FS_Open(va("zs/%s.bsp", filename), "rb", false);
	if (!f) return;*/
	...
	if( lumplen >= 10 )
	{
		...
		/*FS_WriteFile(va("orig/%s.ent", filename), entities, lumplen);
		Msg("Write 'orig/%s.ent'\n", filename);

		FS_WriteFile(va("dm/%s.ent", filename), entities, lumplen);
		Msg("Write 'dm/%s.ent'\n", filename);

		FS_WriteFile(va("tdm/%s.ent", filename), entities, lumplen);
		Msg("Write 'tdm/%s.ent'\n", filename);

		FS_WriteFile(va("gdn/%s.ent", filename), entities, lumplen);
		Msg("Write 'gdm/%s.ent'\n", filename);

		FS_WriteFile(va("zb2/%s.ent", filename), entities, lumplen);
		Msg("Write 'zb2/%s.ent'\n", filename);

		FS_WriteFile(va("zb3/%s.ent", filename), entities, lumplen);
		Msg("Write 'zb3/%s.ent'\n", filename);

		FS_WriteFile(va("zsh/%s.ent", filename), entities, lumplen);
		Msg("Write 'zsh/%s.ent'\n", filename);

		FS_WriteFile(va("zs/%s.ent", filename), entities, lumplen);
		Msg("Write 'zs/%s.ent'\n", filename);*/
...
char *SV_ReadEntityScript( const char *filename, int *flags )
{
	...
	*flags = 0;
	...
	/*Q_strncpy(bspfilename, va("orig/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);

	Q_strncpy(bspfilename, va("dm/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);

	Q_strncpy(bspfilename, va("tdm/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);

	Q_strncpy(bspfilename, va("gdm/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);

	Q_strncpy(bspfilename, va("zb2/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);

	Q_strncpy(bspfilename, va("zb3/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);

	Q_strncpy(bspfilename, va("zsh/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);

	Q_strncpy(bspfilename, va("zs/%s.bsp", filename), sizeof(entfilename));
	f = FS_Open(bspfilename, "rb", false);*/
	...
	*flags |= MAP_IS_EXIST;
	...
	/*Q_strncpy(entfilename, va("orig/%s.ent", filename), sizeof(entfilename));
	Q_strncpy(entfilename, va("dm/%s.ent", filename), sizeof(entfilename));
	Q_strncpy(entfilename, va("tdm/%s.ent", filename), sizeof(entfilename));
	Q_strncpy(entfilename, va("gdm/%s.ent", filename), sizeof(entfilename));
	Q_strncpy(entfilename, va("zb2/%s.ent", filename), sizeof(entfilename));
	Q_strncpy(entfilename, va("zb3/%s.ent", filename), sizeof(entfilename));
	Q_strncpy(entfilename, va("zsh/%s.ent", filename), sizeof(entfilename));
	Q_strncpy(entfilename, va("zs/%s.ent", filename), sizeof(entfilename));*/
