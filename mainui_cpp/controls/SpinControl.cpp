...
CModeSpinControl::CModeSpinControl() : BaseClass(), m_szBackground(),
m_szLeftArrow(), m_szRightArrow(), m_szLeftArrowFocus(), m_szRightArrowFocus(),
m_flMinValue(0), m_flMaxValue(1), m_flCurValue(0), m_flRange(0.1), m_pModel(NULL),
m_iFloatPrecision(0), m_szDisplay()
{
...
=================
*/
void CModeSpinControl::VidInit(void)
{
	colorBase.SetDefault(uiColorHelp);
...
=================
*/
const char* CModeSpinControl::Key(int key, int down)
{
	const char* sound = 0;
	Size arrow;
	Point left, right;
...
=================
*/
void CModeSpinControl::Draw(void)
{
	int	leftFocus, rightFocus;
	Size arrow;
	Point left, right;
...
const char* CModeSpinControl::MoveLeft()
{
	const char* sound;
...
const char* CModeSpinControl::MoveRight()
{
	const char* sound;
...
void CModeSpinControl::UpdateEditable()
{
	switch (m_eType)
	{
	case CVAR_STRING:
...
void CModeSpinControl::Setup(float minValue, float maxValue, float range)
{
	m_flMinValue = minValue;
	m_flMaxValue = maxValue;
	m_flRange = range;
...
void CModeSpinControl::Setup(CMenuBaseArrayModel* model)
{
	m_pModel = model;
	m_flMinValue = 0;
	m_flMaxValue = model->GetRows() - 1;
...
void CModeSpinControl::SetCurrentValue(float curValue)
{
	m_flCurValue = curValue;
	Display();
}

void CModeSpinControl::SetCurrentValue(const char* stringValue)
{
	ASSERT(m_pModel);
...
void CModeSpinControl::SetDisplayPrecision(short precision)
{
	m_iFloatPrecision = precision;
}

void CModeSpinControl::Display()
{
	if (!m_pModel)
	{
		SetCvarValue(m_flCurValue);
...
void CModeSpinControl::ForceDisplayString(const char* display)
{
	Q_strncpy(m_szDisplay, display, CS_SIZE);
}
