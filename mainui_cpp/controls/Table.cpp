...
CMapSet::CMapSet() : BaseClass()
{
	bDrawStroke = true;
}

void CMapSet::VidInit()
{
	BaseClass::VidInit();
...
bool CMapSet::MoveView(int delta)
{
	iTopItem += delta;
...
bool CMapSet::MoveCursor(int delta)
{
	iCurItem += delta;
...
void CMapSet::SetCurrentIndex(int idx)
{
	
}

const char* CMapSet::Key(int key, int down)
{
	int i;
	bool noscroll = false;
...
void CMapSet::DrawLine(Point p, const char** psz, size_t size, uint textColor, bool forceCol, uint fillColor)
{
	size_t i;
	Size sz;
	uint textflags = 0;
...
void CMapSet::DrawLine(Point p, int line, uint textColor, bool forceCol, uint fillColor)
{
	int i;
	Size sz;
