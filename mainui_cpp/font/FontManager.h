...
public:
	...
	float GetLastFontCreationTime() const { return m_flLastFontCreationTime; }
	int GetFontCount() const { return m_Fonts.Count(); }
...
private:
	...
	bool ValidateFontHandle(HFont font) const;

	CUtlVector<CBaseFont*> m_Fonts;
	float m_flLastFontCreationTime; // Track font creation performance

	// Cache for common operations
	mutable int m_iLastFontTall; // Cache for GetFontTall
	mutable HFont m_hLastFont;   // Last accessed font handle
