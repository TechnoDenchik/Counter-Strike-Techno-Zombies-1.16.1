...
#include "game_shared2/perf_counter.h"
...
int CBaseEntity::TakeDamage2(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage2, int bitsDamageType2)
{
	if (!pev->takedamage)
		return 0;
	...
	{
		Vector vecDir = (pev->origin - (pevInflictor->absmin + pevInflictor->absmax) * 0.5).Normalize();
		float flForce = flDamage2 * ((32 * 32 * 72.0) / (pev->size.x * pev->size.y * pev->size.z)) * 5;
	...
	pev->health -= flDamage2;

	if (CBaseEntity::Instance(pevAttacker)->IsPlayer() && flDamage2 > 0.0f) {   //not necessary?
		MESSAGE_BEGIN(MSG_ONE, gmsgHitMsg, NULL, pevAttacker);
		WRITE_LONG((long)flDamage2);
		WRITE_SHORT(ENTINDEX(edict()));
		WRITE_BYTE(0);
		MESSAGE_END();
	}
...
void CBaseEntity::Killed2(entvars_t* pevAttacker, int iGib)
{
	pev->takedamage = DAMAGE_NO;
	pev->deadflag = DEAD_DEAD;
	UTIL_Remove(this);
