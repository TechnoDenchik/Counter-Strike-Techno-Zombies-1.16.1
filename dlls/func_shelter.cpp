...
const char * CShelter::pSpawnObjects[] =
{
	NULL,
	"item_battery",
	"item_healthkit",
...
const char * CShelter::pSoundsWood[] =
{
	"debris/wood1.wav",
	"debris/wood2.wav",
	"debris/wood3.wav"
...
const char * CShelter::pSoundsFlesh[] =
{
	"debris/flesh1.wav",
	"debris/flesh2.wav",
	"debris/flesh3.wav",
...
const char * CShelter::pSoundsMetal[] =
{
	"debris/metal1.wav",
	"debris/metal2.wav",
	"debris/metal3.wav"
...
const char * CShelter::pSoundsConcrete[] =
{
...
const char * CShelter::pSoundsGlass[] =
{
	"debris/glass1.wav",
	"debris/glass2.wav",
	"debris/glass3.wav"
...
const char *CPushable2::m_soundNames[] =
{
	"debris/pushbox1.wav",
	"debris/pushbox2.wav",
	"debris/pushbox3.wav"
...
TYPEDESCRIPTION CShelter::m_SaveData[] =
{
	DEFINE_FIELD(CShelter, m_Material, FIELD_INTEGER),
	DEFINE_FIELD(CShelter, m_Explosion, FIELD_INTEGER),
	DEFINE_FIELD(CShelter, m_angle, FIELD_FLOAT),
	DEFINE_FIELD(CShelter, m_iszGibModel, FIELD_STRING),
	DEFINE_FIELD(CShelter, m_iszSpawnObject, FIELD_STRING),
};

TYPEDESCRIPTION CPushable2::m_SaveData[] =
{
	DEFINE_FIELD(CPushable2, m_maxSpeed, FIELD_FLOAT),
	DEFINE_FIELD(CPushable2, m_soundTime, FIELD_TIME),
};

void CShelter::KeyValue(KeyValueData *pkvd)
{
...
LINK_ENTITY_TO_CLASS(shelter_team_barricade, CShelter);

IMPLEMENT_SAVERESTORE(CShelter, CBaseEntity);
...
const char ** CShelter::MaterialSoundList(Materials precacheMaterial, int &soundCount)
{
...
void CShelter::MaterialSoundPrecache(Materials precacheMaterial)
{
...
void CShelter::MaterialSoundRandom(edict_t *pEdict, Materials soundMaterial, float volume)
{
	int soundCount = 0;
...
void CShelter::BreakTouch(CBaseEntity *pOther)
{
	float flDamage;
...
void CShelter::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	if (IsBreakable())
	{
		pev->angles.y = m_angle;
...
void CShelter::TraceAttack(entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
...
BOOL CShelter::IsBreakable()
{
	return m_Material != matUnbreakableGlass;
}

int CShelter::DamageDecal(int bitsDamageType)
{
	if (m_Material == matGlass)
...
LINK_ENTITY_TO_CLASS(func_pushable2, CPushable2);

IMPLEMENT_SAVERESTORE(CPushable2, CShelter);

void CPushable2::Spawn()
{
...
void CPushable2::KeyValue(KeyValueData *pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "buoyancy"))
	{
		...
		CPushable2::KeyValue(pkvd);
}

// Pull the func_pushable

void CPushable2::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	if (!pActivator || !pActivator->IsPlayer())
	{
		if (pev->spawnflags & SF_PUSH_BREAKABLE)
		{
			this->CPushable2::Use(pActivator, pCaller, useType, value);
		}
...
void CPushable2::Touch(CBaseEntity *pOther)
{
...
void CPushable2::Move(CBaseEntity *pOther, int push)
{
	entvars_t *pevToucher = pOther->pev;
	int playerTouch = 0;
...
int CPushable2::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	if (pev->spawnflags & SF_PUSH_BREAKABLE)
	{
		return CShelter::TakeDamage(pevInflictor, pevAttacker, flDamage, bitsDamageType);
	}
