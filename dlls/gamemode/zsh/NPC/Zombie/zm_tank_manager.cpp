...
class CZMTankManager::impl_t
{
public:
	std::vector<CZMTank*> m_vecEntities;
	...
	void ResizeEntities(size_t new_size)
	{
		...
		std::nth_element(m_vecEntities.begin(), m_vecEntities.begin() + new_size, m_vecEntities.end(), [](CZMTank*a, CZMTank*b) {return a->m_flTimeLastActive > b->m_flTimeLastActive; });
...
CZMTankManager::CZMTankManager() : pimpl(new impl_t())
{
	SetMaxNumOfEntity(256);
	SetAutoGcRatio(0.8f);
}

CZMTankManager::~CZMTankManager() // frees the pimpl
{

}

void CZMTankManager::OnEntityAdd(CZMTank*ent)
{
	pimpl->AutoResizeCheck();
	pimpl->m_vecEntities.push_back(ent);
}

void CZMTankManager::OnEntityRemove(CZMTank*ent)
{
	auto &v = pimpl->m_vecEntities;
	v.erase(std::find(v.begin(), v.end(), ent), v.end());
}

size_t CZMTankManager::EntityCount()
{
	return pimpl->m_vecEntities.size();
}

void CZMTankManager::EntityForEach(std::function<void(CZMTank*)> functor)
{
	auto &v = pimpl->m_vecEntities;
	std::for_each(v.begin(), v.end(), functor);
}

void CZMTankManager::SetMaxNumOfEntity(size_t what)
{
	pimpl->m_iMaxEntitiesNum = what;
}

size_t CZMTankManager::GetMaxNumOfEntity()
{
	return pimpl->m_iMaxEntitiesNum;
}

void CZMTankManager::SetAutoGcRatio(float what)
{
