...
#include "zm_light_manager.h"
#include "gamemode/mods.h"

#include <future>
#include <atomic>

LINK_ENTITY_TO_CLASS(zm_light, CZMLight);

class CZMLightImprov : public CHostageImprov
{
public:
	CZMLightImprov(CBaseEntity* entity) : CHostageImprov(entity),
		m_bCalculatingPath(false)
	{

	}
	~CZMLightImprov() {} // virtual
...
void CZMLight::Spawn()
{
	...
	MAKE_STRING_CLASS("zm_light", pev);
	...
	SetThink(&CZMLight::IdleThink);
	SetThink(&CZMLight::ZM2Think);
	...
	m_pLightStrategy->OnSpawn();
}

void CZMLight::Precache()
{
...
void CZMLight::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	// skip CHostage::Use
	return CBaseMonster::Use(pActivator, pCaller, useType, value);
}

void CZMLight::Touch(CBaseEntity* pOther)
{
	if (m_improv != NULL)
	{
...
float CZMLight::GetModifiedDamage(float flDamage, int nHitGroup) const
{
	switch (nHitGroup)
	{
...
int CZMLight::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
...
void CZMLight::Killed(entvars_t* pevAttacker, int iGib)
{
	if (!IsAlive())
		return;
	...
	SetThink(&CZMLight::Remove);

	m_flTimeLastActive = -1;

	m_pLightStrategy->OnKilled(pevAttacker, iGib);
}

void CZMLight::Remove()
{
...
CZMLight::CZMLight() : CHostage(),
m_iKillBonusMoney(500),
m_iKillBonusFrags(1),
m_flTimeLastActive(0.0f),
m_pLightStrategy(new CZMLightModStrategy_Default(this))
{
	CZMLightManager().OnEntityAdd(this);
}

CZMLight::~CZMLight()
{
	CZMLightManager().OnEntityRemove(this);
}

void CZMLight::IdleThink()
{
	...
	if (!m_improv)
	{
		m_improv = new CZMLightImprov(this);
	}
	...
	std::future<void> handleDispatchAnimEvents = std::async(&CZMLight::DispatchAnimEvents, this, flInterval);
	...
	if (gpGlobals->time > m_flNextFullThink)
	{
		...
		std::future<bool> results[] = {
			std::async(&CZMLight::CheckTarget, this),
			std::async(&CZMLight::CheckAttack, this),
			std::async(&CZMLight::CheckSequence, this),
	...
	m_pLightStrategy->OnThink();
}

void CZMLight::ZM2Think()
{
...
std::pair<CBasePlayer*, bool> CZMLight::FindTarget() const
{
	std::future<CBasePlayer*> fpNear = std::async(&CZMLight::GetClosestPlayer, this, true);
	std::future<CBasePlayer*> fpFar = std::async(&CZMLightImprov::GetClosestPlayerByTravelDistance, m_improv, UNASSIGNED, nullptr);
...
void CZMLight::SetAnimation(TankAnim anim) // similar to CBasePlayer::SetAnimation
{
	int animDesired = 0;
	float speed;
	char szAnim[64];
...
int CZMLight::LookupSequence(const char* label)
{
...
CBaseEntity* CZMLight::CheckTraceHullAttack(float flDist, int iDamage, int iDmgType)
{
	TraceResult tr;
...
bool CZMLight::ShouldAttack(CBaseEntity* target) const
{
	if (target->pev->takedamage == DAMAGE_NO)
		return false;

	if (target->IsPlayer() && !m_pLightStrategy->IsTeamMate(target))
		return true;
...
CBasePlayer* CZMLight::GetClosestPlayer(bool bVisible) const
{
	if (!m_improv)
		return NULL;
...
void CZMLightModStrategy_Default::OnSpawn(){}

void CZMLightModStrategy_Default::OnThink(){}

void CZMLightModStrategy_Default::OnKilled(entvars_t* pKiller, int iGib){}

void CZMLightModStrategy_Default::DeathSound() const
{
	 EMIT_SOUND(ENT(m_pZMLight->pev), CHAN_VOICE, "zbs/zbs_death_female_1.wav", VOL_NORM, ATTN_NORM);
}