...
class CZMLightManager::impl_t
{
public:
	std::vector<CZMLight*> m_vecEntities;
	...
	void ResizeEntities(size_t new_size)
	{
		...
		std::nth_element(m_vecEntities.begin(), m_vecEntities.begin() + new_size, m_vecEntities.end(), [](CZMLight* a, CZMLight* b) {return a->m_flTimeLastActive > b->m_flTimeLastActive; });
...
CZMLightManager::CZMLightManager() : pimpl(new impl_t())
{
	SetMaxNumOfEntity(256);
	SetAutoGcRatio(0.8f);
}

CZMLightManager::~CZMLightManager() // frees the pimpl
{

}

void CZMLightManager::OnEntityAdd(CZMLight* ent)
{
	pimpl->AutoResizeCheck();
	pimpl->m_vecEntities.push_back(ent);
}

void CZMLightManager::OnEntityRemove(CZMLight* ent)
{
	auto& v = pimpl->m_vecEntities;
	v.erase(std::find(v.begin(), v.end(), ent), v.end());
}

size_t CZMLightManager::EntityCount()
{
	return pimpl->m_vecEntities.size();
}

void CZMLightManager::EntityForEach(std::function<void(CZMLight*)> functor)
{
	auto& v = pimpl->m_vecEntities;
	std::for_each(v.begin(), v.end(), functor);
}

void CZMLightManager::SetMaxNumOfEntity(size_t what)
{
	pimpl->m_iMaxEntitiesNum = what;
}

size_t CZMLightManager::GetMaxNumOfEntity()
{
	return pimpl->m_iMaxEntitiesNum;
}

void CZMLightManager::SetAutoGcRatio(float what)
{
