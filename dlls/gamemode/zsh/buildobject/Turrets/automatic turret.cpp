...
Vector vecMid = pev->origin + pev->view_ofs;
	Vector vecMidEnemy = m_hTargetEnt->BodyTarget(vecMid);
	UTIL_MakeVectors(pev->v_angle);
	//Vector vecSrc = GetGunPosition();
	Vector vecEnd = vecMid + gpGlobals->v_forward * 48;
	CBasePlayer* player = NULL;

	pev->nextthink = gpGlobals->time + 0.1;
	StudioFrameAdvance();

	if (m_hTargetEnt->IsAlive() || (m_hTargetEnt = 0))
	{
		m_hTargetEnt = NULL;
		m_flLastSight = gpGlobals->time + m_flMaxWait;
	...
	CBaseEntity* Target = UTIL_FindEntityByTargetname(NULL, STRING(m_hTargetEnt->pev->target));
	...
	TraceResult tr;
	UTIL_TraceLine(vecMidEnemy, vecEnd, dont_ignore_monsters, ENT(pev), &tr);

	if (tr.flFraction >= 1)
	{
		UTIL_TraceHull(vecMidEnemy, vecEnd, dont_ignore_monsters, head_hull, ENT(pev), &tr);
		...
		if (tr.flFraction < 1)
		{
			...
			if (!pHit || pHit->IsBSPModel())
				FindHullIntersection4(vecMidEnemy, tr, VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX, ENT(pev));
	...
	int fEnemyVisible = FBoxVisible(pev, m_hTargetEnt->pev, vecMidEnemy);
