...
struct CResGroup::impl_t
{
	std::vector<std::weak_ptr<CResGroupMember>> m_vecGroupMembers;
	Vector m_moveGoal;
	CNavPath m_path;
...
CResGroup::CResGroup(const Vector &origin)
	: pimpl(new impl_t())
{
...
std::shared_ptr<CResGroupMember> CResGroup::Join()
{
	auto ret = std::make_shared<CResGroupMember>(shared_from_this());
	pimpl->m_vecGroupMembers.emplace_back(ret);
	return ret;
}

bool CResGroup::IsCurrentAt(const Vector &vecWhere) const
{
	return pimpl->m_startArea != nullptr && pimpl->m_startArea->Contains(&vecWhere);
}

bool CResGroupMember::Apply(CHostageImprov* improv)
{
...
{
	// ...
	static std::vector<std::weak_ptr<CResGroup>> TheNpcGroups;

	// optimize
	static int nOptCount = 0;
	if (++nOptCount > 31)
	{
		TheNpcGroups.erase(std::remove_if(TheNpcGroups.begin(), TheNpcGroups.end(), [](const std::weak_ptr<CResGroup> &wp) {
			return wp.expired();
		}), TheNpcGroups.end());
		nOptCount = 0;
	}
	...
	// try to find a group
	std::shared_ptr<CResGroup> spDesiredGroup = nullptr;

	auto iter = std::find_if(TheNpcGroups.begin(), TheNpcGroups.end(), [&start](const std::weak_ptr<CResGroup> &wp) mutable {
		auto sp = wp.lock();
		// expired, remove it
	...
	if (!spDesiredGroup)
	{
		spDesiredGroup = std::make_shared<CResGroup>(start);
