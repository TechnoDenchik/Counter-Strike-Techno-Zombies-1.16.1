...
{
	if (g_vecSpawnRES.empty())
		return false;
	// randomize those fucking spawn points
	std::random_shuffle(g_vecSpawnRES.begin(), g_vecSpawnRES.end());
	using namespace std::placeholders;
	// find the first availble one
	auto iter = std::find_if(g_vecSpawnRES.begin(), g_vecSpawnRES.end(), std::bind(RES_IsSpawnPointValid, pEntity, _1));
	if (iter == g_vecSpawnRES.end())
		return false;

	// sets these item
	RES_ApplyRandomSpawnPoint(pEntity, *iter);
	return true;
}
...
void RES_LoadSpawnPoints()
{
	...
	char filename[256];
	Q_sprintf(filename, "Resource/configs/zsh/%s.resspawns.cst", STRING(gpGlobals->mapname));
	...
	while ((linedata = readline(csdmFile)).first)
	{
		...
		g_vecSpawnRES.emplace_back(MakeSpawnRESPointData(arr));
	}
}
