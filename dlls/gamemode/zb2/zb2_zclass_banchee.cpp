...
void CZombieClass_Banchee::Precache()
{
	PRECACHE_SOUND("sound/zb3/zombi_hurt_banshee_1.wav");
	PRECACHE_SOUND("sound/zb3/zombi_death_banshee_1.wav");
}

CZombieClass_Banchee::CZombieClass_Banchee(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkill.reset(new CZombieSkill_ZombieCrazy(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "witch_zombi_origin" : "witch_zombi_host";
...
void CZombieClass_Banchee::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_BANCHEE);
	WRITE_BYTE(ZOMBIE_SKILL_RATS);
	MESSAGE_END();
}

void CZombieClass_Banchee::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_Banchee::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_Banchee::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_Banchee::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_AUTO, "zb3/zombi_hurt_banshee_1.wav", VOL_NORM, ATTN_NORM);	
}

void CZombieClass_Banchee::DeathSound_Zombie()
{
	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_AUTO, "zb3/zombi_death_banshee_1.wav", VOL_NORM, ATTN_NORM);
}

void CZombieClass_Banchee::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_Banchee::OnThink();
}

void CZombieClass_Banchee::Zombie_HealthRecoveryThink()
{
