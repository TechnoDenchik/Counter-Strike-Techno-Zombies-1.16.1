...
LINK_ENTITY_TO_CLASS(supplyboxh, CSupplyBoxHidden);

void CSupplyBoxHidden::Precache()
{
	PRECACHE_SOUND("zb3/get_box.wav");
	PRECACHE_MODEL("models/supplybox.mdl");
}

void CSupplyBoxHidden::Spawn()
{
	Precache();
	...
	MAKE_STRING_CLASS("supplyboxh", pev);
	...
	SetTouch(&CSupplyBoxHidden::SupplyboxTouch);
	SetThink(&CSupplyBoxHidden::SupplyboxThink);
...
void CSupplyBoxHidden::SupplyboxTouch(CBaseEntity *pOther)
{
	if (!pOther->IsPlayer())
		return;
...
void CSupplyBoxHidden::SupplyboxThink()
{
...
void CSupplyBoxHidden::SendPositionMsg()
{
...
void CSupSpawnHidden::Spawn()
{
	return CPointEntity::Spawn();
}

void CSupSpawnHidden::KeyValue(KeyValueData* pkvd)
{
	
}

BOOL CSupSpawnHidden::IsTriggered(CBaseEntity* pEntity)
{
