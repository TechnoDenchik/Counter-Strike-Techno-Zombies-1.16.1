...
CHeroSkillR::CHeroSkillR(CBasePlayer *player) : BasePlayerExtra(player), m_iZombieSkillStatus(SKILL_STATUS_READY)
{

}

CHeroSkill_sprintR::CHeroSkill_sprintR(CBasePlayer *player) : CHeroSkillR(player)
{

}

void CHeroSkill_sprintR::Think()
{
	CHeroSkill_sprintR::Think();
...
void CHeroSkill_sprintR::Activate()
{
...
void CHeroSkill_sprintR::ResetMaxSpeed()
{
	if (m_iZombieSkillStatus == SKILL_STATUS_USING)
		m_pPlayer->pev->maxspeed = 390;
}

void CHeroSkill_sprintR::OnSkillEnd()
{
...
void CHeroSkill_sprintR::OnCrazyEffect()
{
	m_flTimeZombieSkillEffect = gpGlobals->time + 3.0f;
...
float CHeroSkill_sprintR::GetDurationTime() const
{
	return m_pPlayer->m_iZombieLevel == ZOMBIE_LEVEL_HOST ? 3.0f : 10.0f;
}

float CHeroSkill_sprintR::GetCooldownTime() const
{
	return m_pPlayer->m_iZombieLevel == ZOMBIE_LEVEL_HOST ? 15.0f : 10.0f;
}

float CHeroSkill_sprintR::GetDamageRatio() const
{
	if (m_iZombieSkillStatus == SKILL_STATUS_USING)
		return 1.6f;
	return 1.0f;
