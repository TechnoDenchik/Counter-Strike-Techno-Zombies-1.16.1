...
#include "gamemode/zb5/zb2reference/zb2_zclass_ganimedreference.h"

void CZombieClass_GanimedR::Precache()
{
...
CZombieClass_GanimedR::CZombieClass_GanimedR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
...
void CZombieClass_GanimedR::InitHUD() const
{
...
void CZombieClass_GanimedR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_GanimedR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_GanimedR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_GanimedR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
...
void CZombieClass_GanimedR::DeathSound_Zombie()
{
...
void CZombieClass_GanimedR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_GanimedR::OnThink();
}

void CZombieClass_GanimedR::Zombie_HealthRecoveryThink()
{
