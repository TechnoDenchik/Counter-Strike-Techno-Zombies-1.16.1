...
#include "zb3_zclass_boosterR.h"

void CZombieClass_BoosterR::Precache()
{
	PRECACHE_SOUND("sound/zb3/boosterzombie_booster.wav");
	PRECACHE_SOUND("sound/zb3/boosterzombie_death1.wav");
	PRECACHE_SOUND("sound/zb3/boosterzombie_death2.wav");
	PRECACHE_SOUND("sound/zb3/boosterzombie_hurt1.wav");
	PRECACHE_SOUND("sound/zb3/boosterzombie_hurt2.wav");
	PRECACHE_SOUND("sound/zb3/boosterzombie_stab.wav");
}

CZombieClass_BoosterR::CZombieClass_BoosterR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "booster_zombi_origin" : "booster_zombi_host";
...
void CZombieClass_BoosterR::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_BOOSTER);
	WRITE_BYTE(ZOMBIE_SKILL_BOOSTER);
	WRITE_BYTE(ZOMBIE_SKILL_BOOSTER2);
	MESSAGE_END();
}

void CZombieClass_BoosterR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_BoosterR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_BoosterR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_BoosterR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	switch (RANDOM_LONG(0, 1))
	{
		case 0: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/boosterzombie_hurt1.wav", VOL_NORM, ATTN_NORM); break;
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/boosterzombie_hurt2.wav", VOL_NORM, ATTN_NORM); break;
	}
}

void CZombieClass_BoosterR::DeathSound_Zombie()
{
	switch (RANDOM_LONG(1, 2))
	{
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/boosterzombie_death1.wav", VOL_NORM, ATTN_NORM); break;
		case 2: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/boosterzombie_death2.wav", VOL_NORM, ATTN_NORM); break;
	}
}

void CZombieClass_BoosterR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_BoosterR::OnThink();
}

void CZombieClass_BoosterR::Zombie_HealthRecoveryThink()
{
