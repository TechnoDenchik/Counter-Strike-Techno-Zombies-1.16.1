...
#include "zb3_zclass_flyR.h"

void CZombieClass_FlyR::Precache()
{
	PRECACHE_SOUND("sound/zb3/flyzombie_death1.wav");
	PRECACHE_SOUND("sound/zb3/flyzombie_death2.wav");
	PRECACHE_SOUND("sound/zb3/flyzombie_downhill_start.wav");
	PRECACHE_SOUND("sound/zb3/flyzombie_fly_idle.wav");
	PRECACHE_SOUND("sound/zb3/flyzombie_fly_start.wav");
	PRECACHE_SOUND("sound/zb3/flyzombie_hurt1.wav");
	PRECACHE_SOUND("sound/zb3/flyzombie_hurt2.wav");
}

CZombieClass_FlyR::CZombieClass_FlyR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "flyingzombi_origin" : "flyingzombi_host";
...
void CZombieClass_FlyR::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_FLY);
	WRITE_BYTE(ZOMBIE_SKILL_FLYING);
	WRITE_BYTE(ZOMBIE_SKILL_FLYING3);
	MESSAGE_END();
}

void CZombieClass_FlyR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_FlyR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_FlyR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_FlyR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	switch (RANDOM_LONG(0, 1))
	{
		case 0: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/flyzombie_hurt1.wav", VOL_NORM, ATTN_NORM); break;
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/flyzombie_hurt2.wav", VOL_NORM, ATTN_NORM); break;
	}
}

void CZombieClass_FlyR::DeathSound_Zombie()
{
	switch (RANDOM_LONG(1, 2))
	{
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/flyzombie_death1.wav", VOL_NORM, ATTN_NORM); break;
		case 2: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/flyzombie_death2.wav", VOL_NORM, ATTN_NORM); break;
	}
}

void CZombieClass_FlyR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_FlyR::OnThink();
}

void CZombieClass_FlyR::Zombie_HealthRecoveryThink()
{
