...
#include "zb3_zclass_chinaR.h"

void CZombieClass_ChinaR::Precache()
{
...
CZombieClass_ChinaR::CZombieClass_ChinaR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
...
void CZombieClass_ChinaR::InitHUD() const
{
...
void CZombieClass_ChinaR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_ChinaR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_ChinaR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_ChinaR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/zombi_chinese_hurt.wav", VOL_NORM, ATTN_NORM);
}

void CZombieClass_ChinaR::DeathSound_Zombie()
{
	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/zombi_chinese_death.wav", VOL_NORM, ATTN_NORM);
}

void CZombieClass_ChinaR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_ChinaR::OnThink();
}

void CZombieClass_ChinaR::Zombie_HealthRecoveryThink()
{
