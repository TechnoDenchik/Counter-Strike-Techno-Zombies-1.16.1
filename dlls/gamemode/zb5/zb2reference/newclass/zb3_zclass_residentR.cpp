...
#include "zb3_zclass_residentR.h"

void CZombieClass_ResidentR::Precache()
{
	PRECACHE_SOUND("sound/zb3/resident_death.wav");
	PRECACHE_SOUND("sound/zb3/resident_hurt1.wav");
	PRECACHE_SOUND("sound/zb3/resident_hurt2.wav");
	PRECACHE_SOUND("sound/zb3/resident_skill1.wav");
	PRECACHE_SOUND("sound/zb3/resident_skill2.wav");
	PRECACHE_SOUND("sound/zb3/resident_stab.wav");
	PRECACHE_SOUND("sound/zb3/resident_stab_miss.wav");
	PRECACHE_SOUND("sound/zb3/resident_tw.wav");
}

CZombieClass_ResidentR::CZombieClass_ResidentR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "resident_zombi_origin" : "resident_zombi_host";
...
void CZombieClass_ResidentR::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_RESIDENT);
	WRITE_BYTE(ZOMBIE_SKILL_RESIDENT);
	WRITE_BYTE(ZOMBIE_SKILL_RESIDENT2);
	MESSAGE_END();
}

void CZombieClass_ResidentR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_ResidentR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_ResidentR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_ResidentR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	switch (RANDOM_LONG(0, 1))
	{
		case 0: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/resident_hurt1.wav", VOL_NORM, ATTN_NORM); break;
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/resident_hurt2.wav", VOL_NORM, ATTN_NORM); break;
	}
}

void CZombieClass_ResidentR::DeathSound_Zombie()
{
	 EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "zb3/resident_death.wav", VOL_NORM, ATTN_NORM);
}

void CZombieClass_ResidentR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_ResidentR::OnThink();
}

void CZombieClass_ResidentR::Zombie_HealthRecoveryThink()
{
