...
void CZombieClass_StamperR::Precache()
{
	PRECACHE_SOUND("sound/zb3/zombi_hurt_stamper_1.wav");
	PRECACHE_SOUND("sound/zb3/zombi_hurt_stamper_2.wav");
	PRECACHE_SOUND("sound/zb3/zombi_death_stamper_1.wav");
	PRECACHE_SOUND("sound/zb3/zombi_death_stamper_2.wav");
}

CZombieClass_StamperR::CZombieClass_StamperR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "stamper_zombi_origin" : "stamper_zombi_host";
...
void CZombieClass_StamperR::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_STAMPER);
	WRITE_BYTE(ZOMBIE_SKILL_STAMP);
	MESSAGE_END();
}

void CZombieClass_StamperR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_StamperR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_StamperR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_StamperR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	switch (RANDOM_LONG(0, 1))
	{
		case 0: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_hurt_stamper_1.wav", VOL_NORM, ATTN_NORM); break;
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_hurt_stamper_2.wav", VOL_NORM, ATTN_NORM); break;
		default:break;
	}
}

void CZombieClass_StamperR::DeathSound_Zombie()
{
	switch (RANDOM_LONG(1, 2))
	{
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_death_stamper_1.wav", VOL_NORM, ATTN_NORM); break;
		case 2: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_death_stamper_2.wav", VOL_NORM, ATTN_NORM); break;
		default:break;
	}
}

void CZombieClass_StamperR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_StamperR::OnThink();
}

void CZombieClass_StamperR::Zombie_HealthRecoveryThink()
{
