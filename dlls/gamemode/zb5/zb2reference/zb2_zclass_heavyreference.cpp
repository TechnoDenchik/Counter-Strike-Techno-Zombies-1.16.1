...
void CZombieClass_HeavyR::Precache()
{
	PRECACHE_SOUND("sound/zb3/zombi_hurt_heavy_1.wav");
	PRECACHE_SOUND("sound/zb3/zombi_hurt_heavy_2.wav");
	PRECACHE_SOUND("sound/zb3/zombi_death_heavy_1.wav");
	PRECACHE_SOUND("sound/zb3/zombi_death_heavy_2.wav");
}

CZombieClass_HeavyR::CZombieClass_HeavyR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "heavy_zombi_origin" : "heavy_zombi_host";
	...
	m_pPlayer->GiveNamedItem("weapon_zombibomb_heavy");
	m_pPlayer->m_bIsZombieHeavy = true;
}

void CZombieClass_HeavyR::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_HEAVY);
	WRITE_BYTE(ZOMBIE_SKILL_TRAP);
	MESSAGE_END();
}

void CZombieClass_HeavyR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_HeavyR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_HeavyR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_HeavyR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	switch (RANDOM_LONG(0, 1))
	{
		case 0: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_hurt_heavy_1.wav", VOL_NORM, ATTN_NORM); break;
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_hurt_heavy_2.wav", VOL_NORM, ATTN_NORM); break;
		default:break;
	}
}

void CZombieClass_HeavyR::DeathSound_Zombie()
{
	switch (RANDOM_LONG(1, 2))
	{
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_death_heavy_1.wav", VOL_NORM, ATTN_NORM); break;
		case 2: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_VOICE, "zb3/zombi_death_heavy_2.wav", VOL_NORM, ATTN_NORM); break;
		default:break;
	}
}

void CZombieClass_HeavyR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_HeavyR::OnThink();
}

void CZombieClass_HeavyR::Zombie_HealthRecoveryThink()
{
