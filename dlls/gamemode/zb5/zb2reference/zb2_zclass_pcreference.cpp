...
void CZombieClass_PsychoR::Precache()
{
...
CZombieClass_PsychoR::CZombieClass_PsychoR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "pc_zombi_origin" : "pc_zombi_host";
	...
	m_pPlayer->GiveNamedItem("weapon_zombibomb_pc");
	m_pPlayer->m_bIsZombiePc = true;
}

void CZombieClass_PsychoR::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_PC);
	WRITE_BYTE(ZOMBIE_SKILL_SMOKE);
	MESSAGE_END();
}

void CZombieClass_PsychoR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_PsychoR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_PsychoR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_PsychoR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
...
void CZombieClass_PsychoR::DeathSound_Zombie()
{
...
void CZombieClass_PsychoR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_PsychoR::OnThink();
}

void CZombieClass_PsychoR::Zombie_HealthRecoveryThink()
{
