...
void CZombieClass_Spider::Precache()
{
	PRECACHE_SOUND("sound/zb5/spider_hurt1.wav");
	PRECACHE_SOUND("sound/zb5/spider_hurt2.wav");
	PRECACHE_SOUND("sound/zb5/spider_death1.wav");
	PRECACHE_SOUND("sound/zb5/spider_death2.wav");

	PRECACHE_MODEL("sprites/deathres_zombie.spr");
}

CZombieClass_Spider::CZombieClass_Spider(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = "zombi_spider";
...
void CZombieClass_Spider::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_SPIDER);
	WRITE_BYTE(ZOMBIE_SKILL_SPIDER);
	WRITE_BYTE(ZOMBIE_SKILL_SPIDER2);
	MESSAGE_END();
}

void CZombieClass_Spider::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 390;
}

float CZombieClass_Spider::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_Spider::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
	// knife knockback
...
void CZombieClass_Spider::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
	switch (RANDOM_LONG(0, 1))
	{
		case 0: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_AUTO, "zb5/spider_hurt1.wav", VOL_NORM, ATTN_NORM); break;
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_AUTO, "zb5/spider_hurt2.wav", VOL_NORM, ATTN_NORM); break;
		default:break;
	}
}

void CZombieClass_Spider::DeathSound_Zombie()
{
	switch (RANDOM_LONG(1, 2))
	{
		case 1: EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_AUTO, "zb5/spider_death1.wav", VOL_NORM, ATTN_NORM); break;
...
void CZombieClass_Spider::OnThink()
{
	...
	return CZombieClass_Spider::OnThink();
}


void CZombieClass_Spider::Zombie_HealthRecoveryThink()
{
