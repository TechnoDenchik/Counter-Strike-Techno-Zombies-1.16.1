...
void CZombieClass_SpeedR::Precache()
{
...
CZombieClass_SpeedR::CZombieClass_SpeedR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	...
	m_pPlayer->GiveNamedItem("weapon_zombibomb_speed");
	m_pPlayer->m_bIsZombieFemale = true;
}

void CZombieClass_SpeedR::InitHUD() const
{
...
void CZombieClass_SpeedR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_SpeedR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_SpeedR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
	// knife knockback
...
void CZombieClass_SpeedR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
...
void CZombieClass_SpeedR::DeathSound_Zombie()
{
...
void CZombieClass_SpeedR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_SpeedR::OnThink();
}


void CZombieClass_SpeedR::Zombie_HealthRecoveryThink()
{
