...
void CZombieClass_DeimosR::Precache()
{
...
CZombieClass_DeimosR::CZombieClass_DeimosR(CBasePlayer *player, ZombieLevel iEvolutionLevel) : CBaseZombieClass_ZB2R(player, iEvolutionLevel)
{
	Precache();
	m_pZombieSkillR.reset(new CZombieSkill_ZombieCrazyR(m_pPlayer));

	const char *szModel = iEvolutionLevel ? "deimos_zombi_origin" : "deimos_zombi_host";
	...
	m_pPlayer->GiveNamedItem("weapon_zombibomb_deimos");
	m_pPlayer->m_bIsZombieDeimos = true;
}

void CZombieClass_DeimosR::InitHUD() const
{
	MESSAGE_BEGIN(MSG_ONE, gmsgZB2Msg, nullptr, m_pPlayer->pev);
	WRITE_BYTE(ZB2_MESSAGE_SKILL_INIT);
	WRITE_BYTE(ZOMBIE_CLASS_DEIMOS);
	WRITE_BYTE(ZOMBIE_SKILL_SHOCK);
	MESSAGE_END();
}

void CZombieClass_DeimosR::ResetMaxSpeed() const
{
	m_pPlayer->pev->maxspeed = 310;
}

float CZombieClass_DeimosR::AdjustDamageTaken(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType) const
{
	flDamage *= 0.9;
	return CBaseZombieClass_ZB2R::AdjustDamageTaken(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}

bool CZombieClass_DeimosR::ApplyKnockback(CBasePlayer *attacker, const KnockbackData &kbd)
{
...
void CZombieClass_DeimosR::Pain_Zombie(int m_LastHitGroup, bool HasArmour)
{
...
void CZombieClass_DeimosR::DeathSound_Zombie()
{
...
void CZombieClass_DeimosR::OnThink()
{
	Zombie_HealthRecoveryThink();
	return CZombieClass_DeimosR::OnThink();
}

void CZombieClass_DeimosR::Zombie_HealthRecoveryThink()
{
