...
LINK_ENTITY_TO_CLASS(monster_entity_boss, CMonsterBoss);
...
void CMonsterBoss::Precache()
{
	PRECACHE_MODEL("models/shelter/monster/heavy_boss.mdl");
	PRECACHE_SOUND("sound/zsh/monster/zombie_attack2.wav");
}

void CMonsterBoss::Spawn()
{
	Precache();
	...
	MAKE_STRING_CLASS("monster_entity_boss", pev);
	...
	if (pev->skin < 0)
	 ...
	 SET_MODEL(edict(), "models/shelter/monster/heavy_boss.mdl");
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, "sound/zsh/monster/zombie_attack2.wav", VOL_NORM, ATTN_NORM);

	SetThink(&CMonsterBoss::IdleThink);
	...
	m_flAttackDamage = 112210.0f;
	m_flAttackRate = 2.0f;
	m_flAttackAnimTime = 0.6f;
	m_flTimeLastActive = gpGlobals->time;

	m_pMonsterStrategy3->OnSpawn();
}



void CMonsterBoss::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	// skip CHostage::Use
	return CBaseMonster::Use(pActivator, pCaller, useType, value);
}

void CMonsterBoss::Touch(CBaseEntity* pOther)
{
	if (m_improv != NULL)
	{
...
float CMonsterBoss::GetModifiedDamage(float flDamage, int nHitGroup) const
{
	switch (nHitGroup)
	{
...
int CMonsterBoss::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
...
void CMonsterBoss::BecomeDead(void)
{
	pev->health = 0;
...
void CMonsterBoss::Killed(entvars_t* pevAttacker, int iGib)
{
	if (!IsAlive())
		return;
	...
	SetThink(&CMonsterBoss::Remove);

	m_flTimeLastActive = -1;

	m_pMonsterStrategy3->OnKilled(pevAttacker, iGib);
}

void CMonsterBoss::Remove()
{
...
CMonsterBoss::CMonsterBoss() : CHostage(),
m_iKillBonusMoney(500),
m_iKillBonusFrags(1),
m_flTimeLastActive(0.0f),
m_pMonsterStrategy3(new CMonsterBossStrategy(this))
{
	CBossManager().OnEntityAdd(this);
}

CMonsterBoss::~CMonsterBoss()
{
	BossManager().OnEntityRemove(this);
}

void CMonsterBoss::IdleThink()
{
	...
	std::future<void> handleDispatchAnimEvents = std::async(&CMonsterBoss::DispatchAnimEvents, this, flInterval);
	...
	if (gpGlobals->time > m_flNextFullThink)
	{
		...
		std::future<bool> results[] = {
			std::async(&CMonsterBoss::CheckTarget, this),
			std::async(&CMonsterBoss::CheckAttack, this),
			std::async(&CMonsterBoss::CheckSequence, this),
	...
	m_pMonsterStrategy3->OnThink();
}

bool CMonsterBoss::CheckTarget()
{
...
std::pair<CBasePlayer*, bool> CMonsterBoss::FindTarget() const
{
	std::future<CBasePlayer*> fpNear = std::async(&CMonsterBoss::GetClosestPlayer, this, true);
...
bool CMonsterBoss::CheckSequence()
{
	// sequence settings.
...
void CMonsterBoss::SetAnimation(MonsterAnim anim) // similar to CBasePlayer::SetAnimation
{
	int animDesired = 0;
	float speed;
	char szAnim[64];
	...
	case MONSTERANIM_DIE:
	{
		...
		m_pMonsterStrategy3->DeathSound();
		break;
	}
...
int CMonsterBoss::LookupSequence(const char* label)
{
...
CBaseEntity* CMonsterBoss::CheckTraceHullAttack(float flDist, int iDamage, int iDmgType)
{
	TraceResult tr;
...
bool CMonsterBoss::ShouldAttack(CBaseEntity* target) const
{
	if (target->pev->takedamage == DAMAGE_NO)
		return false;

	if (target->IsPlayer() && !m_pMonsterStrategy3->IsTeamMate(target))
		return true;
...
CBasePlayer* CMonsterBoss::GetClosestPlayer(bool bVisible) const
{
	if (!m_improv)
		return NULL;
...
void CMonsterBossStrategy::OnSpawn()
{

}

void CMonsterBossStrategy::OnThink()
{

}

void CMonsterBossStrategy::OnKilled(entvars_t* pKiller, int iGib)
{

}

void CMonsterBossStrategy::DeathSound() const
{
