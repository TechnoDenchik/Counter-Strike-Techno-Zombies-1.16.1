...
#define WONDERCANNON_CHAIN_DAMAGE_INTERVAL	0.2f
		...
		while ((pNewEntity = UTIL_FindEntityInSphere(pNewEntity, m_pEnemyList->Element(i)->pev->origin, WONDERCANNON_CHAIN_DISTANCE)) != NULL)
		{
			...
			if (m_pAttachedEnt->IsPlayer() && !pev->iuser1)
			{
				...
				if (g_pGameRules->PlayerRelationship(player, pNewEntity) != GR_TEAMMATE)
				{
					...
					MESSAGE_BEGIN(MSG_PVS, gmsgHeadIcon, NULL);
					WRITE_BYTE(19);
			...
			if (m_iCount < m_iExpTime)
			{
				...
				if (m_iType)
				{
					if (m_iCount % 2)
					{
						...
						MESSAGE_BEGIN(MSG_BROADCAST, SVC_TEMPENTITY, m_pEnemyList->Element(i)->pev->origin);
						WRITE_BYTE(TE_EXPLOSION);
						WRITE_COORD(m_pEnemyList->Element(i)->pev->origin.x);
						WRITE_COORD(m_pEnemyList->Element(i)->pev->origin.y);
						WRITE_COORD(m_pEnemyList->Element(i)->pev->origin.z);
