...
class CKnife_Zombi_banchee : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_banchee, CKnife_Zombi_banchee)
...
void CKnife_Zombi_banchee::Precache(void)
{
...
int CKnife_Zombi_banchee::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_banchee::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombiewitch.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_banchee::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_banchee::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_banchee::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_banchee::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_banchee::Smack);
...
int CKnife_Zombi_banchee::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
