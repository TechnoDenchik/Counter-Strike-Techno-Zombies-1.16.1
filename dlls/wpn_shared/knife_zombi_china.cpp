...
class CKnife_Zombi_China : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_china, CKnife_Zombi_China)
...
void CKnife_Zombi_China::Precache(void)
{
	PRECACHE_MODEL("models/v_knife_zombi-chinazombie.mdl");
...
int CKnife_Zombi_China::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_China::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombi-chinazombie.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_China::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_China::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_China::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_China::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_China::Smack);
...
int CKnife_Zombi_China::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
