...
void CStarchaserar::PrimaryAttack(void)
{
	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
		StarchaserarFire(0.035 + (0.4) * m_flAccuracy, 0.0825f, FALSE);
	else if (m_pPlayer->pev->velocity.Length2D() > 140)
		StarchaserarFire(0.035 + (0.07) * m_flAccuracy, 0.0825f, FALSE);
	else if (m_pPlayer->pev->fov == 90)
		StarchaserarFire((0.02) * m_flAccuracy, 0.0825f, FALSE);
	else
		StarchaserarFire((0.02) * m_flAccuracy, 0.135f, FALSE);
}
...
void CStarchaserar::StarchaserarFire(float flSpread, duration_t flCycleTime, BOOL fUseAutoAim)
{
	...
	if (tNextAttack > 1.0f || (gpGlobals->time - tWorldTime > 1.0f) || tDelta > 1.0f)	//可以多射一次
	{
		tNextAttack = 0.0f;
		tDelta = 0.0f;
...
void CStarchaserar::Reload(void)
{
	...
	if (DefaultReload(AUG_MAX_CLIP, STARCHASERAR_RELOAD, 3.3f))
	{
#ifndef CLIENT_DLL
