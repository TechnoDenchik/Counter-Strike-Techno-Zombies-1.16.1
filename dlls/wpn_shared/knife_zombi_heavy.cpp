...
class CKnife_Zombi_heavy : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_heavy, CKnife_Zombi_heavy)
...
void CKnife_Zombi_heavy::Precache(void)
{
...
int CKnife_Zombi_heavy::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_heavy::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombih.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_heavy::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_heavy::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_heavy::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_heavy::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_heavy::Smack);
...
int CKnife_Zombi_heavy::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
