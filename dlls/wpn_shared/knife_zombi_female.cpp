...
class CKnife_Zombi_female : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_female, CKnife_Zombi_female)
...
void CKnife_Zombi_female::Precache(void)
{
...
int CKnife_Zombi_female::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_female::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombis.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_female::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_female::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_female::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_female::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_female::Smack);
...
int CKnife_Zombi_female::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
	...
	else
	{
		...
		ClearMultiDamage();
		if (pEntity)
			pEntity->TraceAttack(m_pPlayer->pev, flDamage + 80, gpGlobals->v_forward, &tr, DMG_NEVERGIB | DMG_BULLET);
		ApplyMultiDamage(m_pPlayer->pev, m_pPlayer->pev);
