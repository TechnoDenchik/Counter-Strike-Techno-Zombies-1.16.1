...
class CKnife_Zombi_Aksha : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_aksha, CKnife_Zombi_Aksha)
...
void CKnife_Zombi_Aksha::Precache(void)
{
...
int CKnife_Zombi_Aksha::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Aksha::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	{
		return DefaultDeploy("models/v_knife_zombiaksha_host.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
	else
	{
		return DefaultDeploy("models/v_knife_zombiaksha.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
}

void CKnife_Zombi_Aksha::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Aksha::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Aksha::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Aksha::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_Aksha::Smack);
...
int CKnife_Zombi_Aksha::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
