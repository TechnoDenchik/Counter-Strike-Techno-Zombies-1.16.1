...
class CKnife_Zombi_Spider : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(knife_zombi_spider, CKnife_Zombi_Spider)
...
void CKnife_Zombi_Spider::Precache(void)
{
...
int CKnife_Zombi_Spider::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Spider::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombispider.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_Spider::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Spider::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Spider::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Spider::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_Spider::Smack);
...
int CKnife_Zombi_Spider::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
