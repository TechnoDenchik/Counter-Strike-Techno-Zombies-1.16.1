...
class CKnife_Zombi_heal : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_heal, CKnife_Zombi_heal)
...
void CKnife_Zombi_heal::Precache(void)
{
...
int CKnife_Zombi_heal::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_heal::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombihealer.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_heal::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_heal::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_heal::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_heal::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_heal::Smack);
...
int CKnife_Zombi_heal::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
