...
class CKnife_Zombi_Resident : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_resident, CKnife_Zombi_Resident)
...
void CKnife_Zombi_Resident::Precache(void)
{
...
int CKnife_Zombi_Resident::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Resident::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombieresident.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_Resident::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Resident::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Resident::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Resident::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_Resident::Smack);
...
int CKnife_Zombi_Resident::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
