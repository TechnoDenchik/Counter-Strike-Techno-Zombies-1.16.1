...
class CKnife_Zombi_Deathknight : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(knife_zombi_deathknight, CKnife_Zombi_Deathknight)
...
void CKnife_Zombi_Deathknight::Precache(void)
{
	...
	PRECACHE_SOUND("zb5/deathknight_slash.wav");
	PRECACHE_SOUND("zb5/deathknight_stab.wav");

	PRECACHE_SOUND("zb5/deathknight_slash_wall.wav");

	m_usKnife = PRECACHE_EVENT(1, "events/knife.sc");
}

int CKnife_Zombi_Deathknight::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Deathknight::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombideathknight.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_Deathknight::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Deathknight::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Deathknight::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Deathknight::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_Deathknight::Smack);
...
int CKnife_Zombi_Deathknight::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
	...
	if (tr.flFraction >= 1)
	{
		if (fFirst)
		{
			...
			switch (RANDOM_LONG(0, 1))
			{
			case 0: EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "zb5/deathknight_slash.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM); break;
			case 1: EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "zb5/deathknight_stab.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM); break;
			}
	...
	else
	{
		...
		if (pEntity)
		{
			if (pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE)
			{
				switch (RANDOM_LONG(0, 2))
				{
				case 0: EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "zb3/zombi_attack_1.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM); break;
				case 1: EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "zb3/zombi_attack_2.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM); break;
				case 2: EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "zb3/zombi_attack_3.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM); break;
				}
				m_pPlayer->m_iWeaponVolume = KNIFE_BODYHIT_VOLUME;
		...
		{
			TEXTURETYPE_PlaySound(&tr, vecSrc, vecSrc + (vecEnd - vecSrc) * 2, BULLET_PLAYER_CROWBAR);
			EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "zb5/deathknight_slash_wall.wav", VOL_NORM, ATTN_NORM, 0, 98 + RANDOM_LONG(0, 3)); 
		}
#endif
