...
{
	ResetRunIdle();
	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.17f;
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.01f;
...
void CDesperado::SecondaryAttack(void)
{
	...
	if (pev->iuser1 != 1)
	{
		...
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.17f;
		m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.17f;
		m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.17f;
		ResetRunIdle();
		return;
	}
...
{
	m_iShotsFired++;
	m_flAccuracy -= (0.3 - ((gpGlobals->time - m_flLastFire) / 1)) * 0.2;
...
void CDesperado::Reload(void)
{
	...
	if (m_flNextPrimaryAttack <= 0.0f && m_flNextSecondaryAttack <= 0.0f)
	{
		if (DefaultReload(DESPERADO_MAX_CLIP, pev->iuser1 ? ANIM_RELOAD_W : ANIM_RELOAD_M, 0.47f))
		{
			ResetRunIdle();
#ifndef CLIENT_DLL
	...
	if (m_flTimeWeaponIdle < UTIL_WeaponTimeBase())
	{
		...
		else
		{
			...
			else if( m_flNextRunEnd<= gpGlobals->time)
			{
				...
				m_flNextRunEnd = gpGlobals->time + 3.0f;
			}
			m_flNextRunIdle = gpGlobals->time;
		}
	}
