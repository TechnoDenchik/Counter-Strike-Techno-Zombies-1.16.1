...
class CKnife_Zombi_stamper : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_stamper, CKnife_Zombi_stamper)
...
void CKnife_Zombi_stamper::Precache(void)
{
...
int CKnife_Zombi_stamper::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_stamper::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombiundertaker.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_stamper::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_stamper::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_stamper::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_stamper::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_stamper::Smack);
...
int CKnife_Zombi_stamper::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
