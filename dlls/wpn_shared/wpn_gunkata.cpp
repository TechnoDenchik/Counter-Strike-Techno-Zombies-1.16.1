...
{
public:
	static constexpr const auto &DefaultReloadTime = 2.3f;
	static constexpr const auto &DefaultDeployTime = 0.75f;
	...
	BOOL Deploy(void) override
	{
		...
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 1.03f;
		return iDeployAnim;
	}
	...
	void Fire(float flSpread, duration_t flCycleTime, BOOL fUseAutoAim)
	{
		...
		if(!((MaxClip - m_iClip) % 3))
		{
			...
			flCycleTime = 0.31f; // this+148
			m_pPlayer->m_flNextAttack = 0.31f;
			m_flAccuracy = AccuracyDefault; //*(_DWORD *)(this + 228) = 1.1;
			++iShootAnim; //++v8;
			m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.53f; // this+156  //v9 = sub_105E0FB0() + 0.52999997; *(float *)(this + 156) = v9;
		}
		...
		switch(m_iMode_pev_iuser1)
		{
			...
			case GUNKATA_MODE_B2:
			{
				...
				else // m_iClip <= 0
				{
					...
					m_flNextSpecialAttack8_pev_dmg_take = 0.63f;
					m_flNextSpecialAttack9_pev_dmg_save = 0.63f;
					m_flNextSpecialAttack10_pev_dmgtime = 1.1f;
					m_flNextSpecialAttack11_pev_speed = 1.1f;
			...
			case GUNKATA_MODE_B3:
			{
				//if(get_gametime() > m_flNextSpecialAttack8_pev_dmg_take)
				{
					...
					m_flNextSpecialAttack8_pev_dmg_take = 0x7F7FFFFF; // 0x7F7FFFFF
				...
				//if(get_gametime() > m_flNextSpecialAttack9_pev_dmg_save)
				{
					...
					{
						m_flNextSpecialAttack9_pev_dmg_save = 0x7F7FFFFF; // 0x7F7FFFFF
					}
				}
