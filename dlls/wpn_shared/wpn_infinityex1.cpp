...
void CInfinityEX1::PrimaryAttack(void)
{
	...
	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
		InfinityFire((0.12) * m_flAccuracy, 0.2, FALSE);
	else if (m_pPlayer->pev->velocity.Length2D() > 0)
		InfinityFire((0.055) * m_flAccuracy, 0.2, FALSE);
	else if (FBitSet(m_pPlayer->pev->flags, FL_DUCKING))
		InfinityFire((0.02) * m_flAccuracy, 0.2, FALSE);
	else
...
void CInfinityEX1::InfinityFire(float flSpread, float flCycleTime, BOOL fUseAutoAim)
{
	m_iShotsFired++;
...
void CInfinityEX1::InfinityFire2(float flSpread, float flCycleTime, BOOL fUseAutoAim)
{
	m_iShotsFired++;
