...
class CKnife_Zombi_Fly : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_fly, CKnife_Zombi_Fly)
...
void CKnife_Zombi_Fly::Precache(void)
{
	PRECACHE_MODEL("models/v_knife_zombieflying.mdl");
	PRECACHE_MODEL("models/v_knife_zombieflying_host.mdl");
...
int CKnife_Zombi_Fly::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Fly::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	{
		return DefaultDeploy("models/v_knife_zombieflying_host.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
	else
	{
		return DefaultDeploy("models/v_knife_zombieflying.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
}

void CKnife_Zombi_Fly::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Fly::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Fly::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Fly::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		if (pEntity)
		{
			if (m_flNextPrimaryAttack + 0.4 < UTIL_WeaponTimeBase())
				pEntity->TraceAttack(m_pPlayer->pev, 450, gpGlobals->v_forward, &tr, DMG_NEVERGIB | DMG_BULLET);
			else
		...
		SetThink(&CKnife_Zombi_Fly::Smack);
...
int CKnife_Zombi_Fly::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
