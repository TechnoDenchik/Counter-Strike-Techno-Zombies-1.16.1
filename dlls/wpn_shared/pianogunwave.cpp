...
#define PIANOGUNEX_SpeedUp 270.0
		...
		class CPianoGunWave : public LinkWeaponTemplate < CPianoGunWave,
		TGeneralData,
...
LINK_ENTITY_TO_CLASS(pianogun_wave, CPianoGunWave)
	...
	void CPianoGunWave::Spawn(void)
	{
		...
		case 1:
		{
			m_flRemoveTime = gpGlobals->time + 0.6f - 0.2f;
		...
		case 3:
		{
			m_flRemoveTime = gpGlobals->time + 0.6f - 0.3f;
		...
		case 10:
		{
			m_flRemoveTime = gpGlobals->time + 3.0f;
			...
			if (pEntityOwner)
			{
				...
				if (pOther->IsPlayer())
				{
					...
					if (player->m_bIsZombie)
					{
						...
						if (strategy)
						{
							//strategy->SetStunSpeedTime(2.0f);
							//strategy->SetStunGravityTime(2.0f);
						}
					}
				}
			}
