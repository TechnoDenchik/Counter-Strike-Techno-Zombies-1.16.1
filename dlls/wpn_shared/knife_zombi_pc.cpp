...
class CKnife_Zombi_psycho : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_psycho, CKnife_Zombi_psycho)
...
void CKnife_Zombi_psycho::Precache(void)
{
...
int CKnife_Zombi_psycho::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_psycho::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombipc.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_psycho::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_psycho::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_psycho::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_psycho::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_psycho::Smack);
...
int CKnife_Zombi_psycho::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
