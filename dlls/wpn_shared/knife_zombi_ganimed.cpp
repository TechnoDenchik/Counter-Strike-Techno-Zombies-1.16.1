...
class CKnife_Zombi_ganimed : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_ganimed, CKnife_Zombi_ganimed)
...
void CKnife_Zombi_ganimed::Precache(void)
{
...
int CKnife_Zombi_ganimed::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_ganimed::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombideimos2.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_ganimed::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_ganimed::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_ganimed::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_ganimed::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_ganimed::Smack);
...
int CKnife_Zombi_ganimed::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
