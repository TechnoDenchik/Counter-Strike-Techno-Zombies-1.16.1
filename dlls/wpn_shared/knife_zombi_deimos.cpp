...
class CKnife_Zombi_deimos : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_deimos, CKnife_Zombi_deimos)
...
void CKnife_Zombi_deimos::Precache(void)
{
...
int CKnife_Zombi_deimos::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_deimos::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
...
void CKnife_Zombi_deimos::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_deimos::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_deimos::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_deimos::Smack);
...
int CKnife_Zombi_deimos::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
