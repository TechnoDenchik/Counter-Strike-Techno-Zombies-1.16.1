...
class CKnife_Zombi_Meatwall : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(knife_zombi_meatwall, CKnife_Zombi_Meatwall)
...
void CKnife_Zombi_Meatwall::Precache(void)
{
...
int CKnife_Zombi_Meatwall::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Meatwall::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	return DefaultDeploy("models/v_knife_zombimeatwall.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnife_Zombi_Meatwall::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Meatwall::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Meatwall::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Meatwall::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		if (pEntity)
		{
			if (m_flNextPrimaryAttack + 0.4 < UTIL_WeaponTimeBase())
				pEntity->TraceAttack(m_pPlayer->pev, 700, gpGlobals->v_forward, &tr, DMG_NEVERGIB | DMG_BULLET);
			else
		...
		SetThink(&CKnife_Zombi_Meatwall::Smack);
...
int CKnife_Zombi_Meatwall::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
