...
LINK_ENTITY_TO_CLASS(knife_snakesword, CSnakeSword)
...
void CSnakeSword::Precache(void)
{
	...
	PRECACHE_SOUND("weapons/snakesword_draw.wav");


	m_usKnife = PRECACHE_EVENT(1, "events/knife.sc");
}

int CSnakeSword::GetItemInfo(ItemInfo *p)
{
...
#endif
		return DefaultDeploy("models/v_snakesword.mdl", "models/p_snakesword.mdl", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CSnakeSword::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.5;
}

void CSnakeSword::WeaponAnimation(int iAnimation)
{
	int flags;
...
void SFindHullIntersection(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CSnakeSword::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CSnakeSword::SetPlayerShieldAnim(void)
{
...
void CSnakeSword::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CSnakeSword::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;
...
void CSnakeSword::SecondaryAttack(void)
{
...
void CSnakeSword::Smack(void)
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR, false, m_pPlayer->pev, false);
}

void CSnakeSword::SwingAgain(void)
{
	Swing(FALSE);
}

void CSnakeSword::WeaponIdle(void)
{
	ResetEmptySound();
...
int CSnakeSword::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
...
int CSnakeSword::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
