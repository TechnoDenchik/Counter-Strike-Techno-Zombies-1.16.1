...
LINK_ENTITY_TO_CLASS(knife_katanad, CKnifeDualKatana)
...
void CKnifeDualKatana::Spawn(void)
{
	Precache();
	m_iId = WEAPON_KNIFE;
...
void CKnifeDualKatana::Precache(void)
{
	...
	PRECACHE_SOUND("weapons/katanad_draw.wav");
	PRECACHE_SOUND("weapons/katanad_hit1.wav");
	PRECACHE_SOUND("weapons/katanad_hit2.wav");
	PRECACHE_SOUND("weapons/katanad_slash1.wav");
	PRECACHE_SOUND("weapons/katanad_slash2.wav");
	PRECACHE_SOUND("weapons/katanad_slash3.wav");
	PRECACHE_SOUND("weapons/katanad_stab.wav");
	PRECACHE_SOUND("weapons/katanad_stab_miss.wav");

	m_usKnife = PRECACHE_EVENT(1, "events/knife.sc");
}

int CKnifeDualKatana::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnifeDualKatana::Deploy(void)
{
...
#endif
		return DefaultDeploy("models/v_katanad.mdl", "models/p_katanad.mdl", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnifeDualKatana::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.5;
}

void CKnifeDualKatana::WeaponAnimation(int iAnimation)
{
	int flags;
...
void DUALFindHullIntersection(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CKnifeDualKatana::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnifeDualKatana::SetPlayerShieldAnim(void)
{
...
void CKnifeDualKatana::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CKnifeDualKatana::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;
...
void CKnifeDualKatana::SecondaryAttack(void)
{
...
void CKnifeDualKatana::Smack(void)
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR, false, m_pPlayer->pev, false);
}

void CKnifeDualKatana::SwingAgain(void)
{
	Swing(FALSE);
}

void CKnifeDualKatana::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnifeDualKatana::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnifeDualKatana::Smack);
...
int CKnifeDualKatana::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
