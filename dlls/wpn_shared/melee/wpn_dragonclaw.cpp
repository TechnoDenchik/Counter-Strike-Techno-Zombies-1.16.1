...
LINK_ENTITY_TO_CLASS(knife_dragontail, CKnifeDragonClaw)
...
void CKnifeDragonClaw::Spawn(void)
{
	Precache();
	m_iId = WEAPON_KNIFE;
...
void CKnifeDragonClaw::Precache(void)
{
	...
	PRECACHE_MODEL("models/ef_dragontail.mdl");

	PRECACHE_SOUND("weapons/dragontail_draw.wav");
	PRECACHE_SOUND("weapons/dragontail_hit1.wav");
	PRECACHE_SOUND("weapons/dragontail_hitwall1.wav");
	PRECACHE_SOUND("weapons/dragontail_idle.wav");
	PRECACHE_SOUND("weapons/dragontail_slash1_1.wav");
	PRECACHE_SOUND("weapons/dragontail_slash1_2.wav");
	PRECACHE_SOUND("weapons/dragontail_slash2_1.wav");
	PRECACHE_SOUND("weapons/dragontail_slash2_2.wav");
	PRECACHE_SOUND("weapons/dragontail_stab_hit.wav");
	PRECACHE_SOUND("weapons/dragontail_stab_miss.wav");
	PRECACHE_SOUND("weapons/dragontail_stab_ready.wav");

	m_usKnife = PRECACHE_EVENT(1, "events/knife.sc");
}

int CKnifeDragonClaw::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnifeDragonClaw::Deploy(void)
{
...
#endif
		return DefaultDeploy("models/v_dragontail.mdl", "models/p_dragontail.mdl", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnifeDragonClaw::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.5;
}

void CKnifeDragonClaw::WeaponAnimation(int iAnimation)
{
	int flags;
...
void DFindHullIntersection(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CKnifeDragonClaw::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnifeDragonClaw::SetPlayerShieldAnim(void)
{
...
void CKnifeDragonClaw::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CKnifeDragonClaw::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;
...
void CKnifeDragonClaw::SecondaryAttack(void)
{
...
void CKnifeDragonClaw::Smack(void)
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR, false, m_pPlayer->pev, false);
}

void CKnifeDragonClaw::SwingAgain(void)
{
	Swing(FALSE);
}

void CKnifeDragonClaw::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnifeDragonClaw::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnifeDragonClaw::Smack);
...
int CKnifeDragonClaw::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
