...
LINK_ENTITY_TO_CLASS(knife_zsh_clawhammer, CKnifeClawHammer)
...
void CKnifeClawHammer::Spawn(void)
{
	Precache();
	m_iId = WEAPON_KNIFE;
	SET_MODEL(ENT(pev), "models/w_zsh_clawhammer.mdl");
...
#endif
	PRECACHE_MODEL("models/w_zsh_clawhammer.mdl");
...
int CKnifeClawHammer::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnifeClawHammer::Deploy(void)
{
...
#endif
		return DefaultDeploy("models/v_zsh_clawhammer.mdl", "models/p_zsh_clawhammer.mdl", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CKnifeClawHammer::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.5;
}

void CKnifeClawHammer::WeaponAnimation(int iAnimation)
{
	int flags;
...
void CFindHullIntersection(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CKnifeClawHammer::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnifeClawHammer::SetPlayerShieldAnim(void)
{
...
void CKnifeClawHammer::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CKnifeClawHammer::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;
...
void CKnifeClawHammer::SecondaryAttack(void)
{
...
void CKnifeClawHammer::Smack(void)
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR, false, m_pPlayer->pev, false);
}

void CKnifeClawHammer::SwingAgain(void)
{
	Swing(FALSE);
}

void CKnifeClawHammer::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnifeClawHammer::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnifeClawHammer::Smack);
...
int CKnifeClawHammer::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
