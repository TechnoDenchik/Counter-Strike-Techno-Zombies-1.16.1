...
LINK_ENTITY_TO_CLASS(knife_stormgiant, CStormGiant)
...
void CStormGiant::Spawn(void)
{
	Precache();
	m_iId = WEAPON_KNIFE;
	SET_MODEL(ENT(pev), "models/w_stormgiant.mdl");
...
void CStormGiant::Precache(void)
{
...
#endif
	PRECACHE_MODEL("models/w_stormgiant.mdl");

	PRECACHE_MODEL("sprites/ef_coilmg.spr");
	...
	m_usKnife = PRECACHE_EVENT(1, "events/stormgiant.sc");
}

int CStormGiant::GetItemInfo(ItemInfo *p)
{
...
#endif
		return DefaultDeploy("models/v_stormgiant.mdl", "models/p_stormgiant.mdl", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
}

void CStormGiant::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.5;
}

void CStormGiant::WeaponAnimation(int iAnimation)
{
	int flags;
...
void STFindHullIntersection(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CStormGiant::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CStormGiant::SetPlayerShieldAnim(void)
{
...
void CStormGiant::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CStormGiant::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;
...
void CStormGiant::SecondaryAttack(void)
{
...
void CStormGiant::Smack(void)
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR, false, m_pPlayer->pev, false);
}

void CStormGiant::SwingAgain(void)
{
	Swing(FALSE);
}

void CStormGiant::WeaponIdle(void)
{
	ResetEmptySound();
...
int CStormGiant::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
...
int CStormGiant::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
