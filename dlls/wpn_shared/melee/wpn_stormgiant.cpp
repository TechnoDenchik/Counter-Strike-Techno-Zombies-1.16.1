...
{
public:
	static constexpr const char *ClassName = "knife_stormgiant";
	static constexpr const char *V_Model = "models/v_stormgiant.mdl";
	static constexpr const char *P_Model = "models/p_stormgiant.mdl";
	...
	static constexpr const auto &DefaultDeployTime = 0.0f;
...
public:
	...
	BOOL Deploy() override
	{
		...
		pev->nextthink = gpGlobals->time + 0.23f;

		BOOL ret = Base::Deploy();

		m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 1.23f;
...
LINK_ENTITY_TO_CLASS(knife_stormgiant, CKnifeStormgiantX)
...
void CKnifeStormgiantX::PrimaryAttack(void)
{
	...
	pev->nextthink = gpGlobals->time + 1.033f;
...
void CKnifeStormgiantX::DeployComboAttackCheck()
{
	...
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 1.17f;
