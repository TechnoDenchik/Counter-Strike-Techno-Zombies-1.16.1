...
LINK_ENTITY_TO_CLASS(knife_thanatos9, CThanatos9)
...
int CThanatos9::GetItemInfo(ItemInfo *p)
{
...
void CThanatos9::WeaponAnimation(int iAnimation)
{
	int flags;
...
void TFindHullIntersection(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CThanatos9::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CThanatos9::SetPlayerShieldAnim(void)
{
...
void CThanatos9::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CThanatos9::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;
...
void CThanatos9::SecondaryAttack(void)
{
...
void CThanatos9::Smack(void)
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR, false, m_pPlayer->pev, false);
}

void CThanatos9::SwingAgain(void)
{
	Swing(FALSE);
}

void CThanatos9::WeaponIdle(void)
{
	ResetEmptySound();
	...
	if (m_pPlayer->m_bShieldDrawn != true)
	{
		...
		SendWeaponAnim(KNIFE_IDLEA, UseDecrement() != FALSE);
	}
}

int CThanatos9::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
...
int CThanatos9::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
