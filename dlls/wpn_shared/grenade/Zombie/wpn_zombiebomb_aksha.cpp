...
#include "player.h"
#include "weapons.h"
#include "wpn_zombiebomb_aksha.h"

LINK_ENTITY_TO_CLASS(weapon_zombibomb_aksha, CZombieBombAksha)

void CZombieBombAksha::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_zombibomb_aksha");
	Precache();
...
void CZombieBombAksha::Precache(void)
{
...
int CZombieBombAksha::GetItemInfo(ItemInfo *p)
{
...
BOOL CZombieBombAksha::Deploy(void)
{
	m_flReleaseThrow = 360;
	m_fMaxSpeed = 250;
	m_iWeaponState &= ~WPNSTATE_SHIELD_DRAWN;
...
BOOL CZombieBombAksha::CanHolster(void)
{
	return CBasePlayerWeapon::CanHolster();
}

void CZombieBombAksha::Holster(int skiplocal)
{
...
void CZombieBombAksha::PrimaryAttack(void)
{
...
void CZombieBombAksha::SetPlayerShieldAnim(void)
{
...
void CZombieBombAksha::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CZombieBombAksha::ShieldSecondaryFire(int up_anim, int down_anim)
{
...
void CZombieBombAksha::SecondaryAttack(void)
{
	ShieldSecondaryFire(SHIELDGUN_DRAW, SHIELDGUN_DRAWN_IDLE);
}

void CZombieBombAksha::WeaponIdle(void)
{
...
BOOL CZombieBombAksha::CanDeploy(void)
{
