...
#include "player.h"
#include "weapons.h"
#include "wpn_zombiebomb_fly.h"

LINK_ENTITY_TO_CLASS(weapon_zombibomb_fly, CZombieBombFly)

void CZombieBombFly::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_zombibomb_fly");
	Precache();
...
void CZombieBombFly::Precache(void)
{
...
int CZombieBombFly::GetItemInfo(ItemInfo *p)
{
...
BOOL CZombieBombFly::Deploy(void)
{
	m_flReleaseThrow = 360;
	m_fMaxSpeed = 250;
	m_iWeaponState &= ~WPNSTATE_SHIELD_DRAWN;
...
BOOL CZombieBombFly::CanHolster(void)
{
	return CBasePlayerWeapon::CanHolster();
}

void CZombieBombFly::Holster(int skiplocal)
{
...
void CZombieBombFly::PrimaryAttack(void)
{
...
void CZombieBombFly::SetPlayerShieldAnim(void)
{
...
void CZombieBombFly::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CZombieBombFly::ShieldSecondaryFire(int up_anim, int down_anim)
{
...
void CZombieBombFly::SecondaryAttack(void)
{
	ShieldSecondaryFire(SHIELDGUN_DRAW, SHIELDGUN_DRAWN_IDLE);
}

void CZombieBombFly::WeaponIdle(void)
{
...
BOOL CZombieBombFly::CanDeploy(void)
{
