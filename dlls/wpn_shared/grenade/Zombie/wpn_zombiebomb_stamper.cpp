...
#include "player.h"
#include "weapons.h"
#include "wpn_zombiebomb_stamper.h"

LINK_ENTITY_TO_CLASS(weapon_zombibomb_stamper, CZombieBombStamper)

void CZombieBombStamper::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_zombibomb_stamper");
	Precache();
	...
	m_flStartThrow = 360;
	m_flReleaseThrow = 360;
	m_iWeaponState &= ~WPNSTATE_SHIELD_DRAWN;

	FallInit();
}

void CZombieBombStamper::Precache(void)
{
	PRECACHE_MODEL("models/v_zombibomb_undertaker_trans.mdl");
	...
	PRECACHE_SOUND("zb3/zombi_bomb_idle_3.wav");
	PRECACHE_SOUND("zb3/zombi_bomb_idle_4.wav");
	PRECACHE_SOUND("zb3/zombi_bomb_pull_1.wav");
	PRECACHE_SOUND("zb3/zombi_bomb_throw.wav");

	m_usCreateExplosion = PRECACHE_EVENT(1, "events/createexplo.sc");
}

int CZombieBombStamper::GetItemInfo(ItemInfo *p)
{
...
BOOL CZombieBombStamper::Deploy(void)
{
	m_flReleaseThrow = 360;
	m_fMaxSpeed = 250;
	m_iWeaponState &= ~WPNSTATE_SHIELD_DRAWN;
	...
	return DefaultDeploy("models/v_zombibomb_undertaker_trans.mdl", "models/p_zombibomb.mdl", HEGRENADE_DRAW, "grenade", UseDecrement() != FALSE);
}

BOOL CZombieBombStamper::CanHolster(void)
{
	return CBasePlayerWeapon::CanHolster();
}

void CZombieBombStamper::Holster(int skiplocal)
{
...
void CZombieBombStamper::PrimaryAttack(void)
{
	if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
		return;

	if (m_flStartThrow == 360 && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] > 0)
	{
...
void CZombieBombStamper::SetPlayerShieldAnim(void)
{
...
void CZombieBombStamper::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CZombieBombStamper::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;

	if (m_flStartThrow != 350)
		return false;
...
void CZombieBombStamper::SecondaryAttack(void)
{
	ShieldSecondaryFire(SHIELDGUN_DRAW, SHIELDGUN_DRAWN_IDLE);
}

void CZombieBombStamper::WeaponIdle(void)
{
	if (m_flReleaseThrow == 360 && m_flStartThrow != 360)
		m_flReleaseThrow = gpGlobals->time;

	if (m_flTimeWeaponIdle > UTIL_WeaponTimeBase())
		return;

	if (m_flStartThrow != 360)
	{
#ifndef CLIENT_DLL
...
BOOL CZombieBombStamper::CanDeploy(void)
{
