...
#include "player.h"
#include "weapons.h"
#include "wpn_zombiebomb_pc.h"

LINK_ENTITY_TO_CLASS(weapon_zombibomb_pc, CZombieBombPc)

void CZombieBombPc::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_zombibomb_pc");
	Precache();
...
void CZombieBombPc::Precache(void)
{
	PRECACHE_MODEL("models/v_zombibombpc_trans.mdl");
...
int CZombieBombPc::GetItemInfo(ItemInfo *p)
{
...
BOOL CZombieBombPc::Deploy(void)
{
	m_flReleaseThrow = 360;
	m_fMaxSpeed = 250;
	m_iWeaponState &= ~WPNSTATE_SHIELD_DRAWN;
	...
	return DefaultDeploy("models/v_zombibombpc_trans.mdl", "models/p_zombibomb.mdl", HEGRENADE_DRAW, "grenade", UseDecrement() != FALSE);
}

BOOL CZombieBombPc::CanHolster(void)
{
	return CBasePlayerWeapon::CanHolster();
}

void CZombieBombPc::Holster(int skiplocal)
{
...
void CZombieBombPc::PrimaryAttack(void)
{
...
void CZombieBombPc::SetPlayerShieldAnim(void)
{
...
void CZombieBombPc::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CZombieBombPc::ShieldSecondaryFire(int up_anim, int down_anim)
{
...
void CZombieBombPc::SecondaryAttack(void)
{
	ShieldSecondaryFire(SHIELDGUN_DRAW, SHIELDGUN_DRAWN_IDLE);
}

void CZombieBombPc::WeaponIdle(void)
{
...
BOOL CZombieBombPc::CanDeploy(void)
{
