...
void CSgdrill::PrimaryAttack(void)
{
	...
	m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.32f;
	m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.32f;
	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.875f;
...
void CSgdrill::Reload(void)
{
	...
	if (DefaultReload(SGDRILL_MAX_CLIP, SGDRILL_RELOAD, 2.9f))
	{
#ifndef CLIENT_DLL
...
void CSgdrill::SecondaryAttack(void)
{
	...
	EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/sgdrill_slash.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
