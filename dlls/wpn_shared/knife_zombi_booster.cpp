...
class CKnife_Zombi_Booster : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_booster, CKnife_Zombi_Booster)
...
void CKnife_Zombi_Booster::Precache(void)
{
	PRECACHE_MODEL("models/v_knife_zombibooster.mdl");
	PRECACHE_MODEL("models/v_knife_zombibooster_host.mdl");
...
int CKnife_Zombi_Booster::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Booster::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	{
		return DefaultDeploy("models/v_knife_zombibooster_host.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
	else
	{
		return DefaultDeploy("models/v_knife_zombibooster.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
}

void CKnife_Zombi_Booster::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Booster::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Booster::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Booster::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_Booster::Smack);
...
int CKnife_Zombi_Booster::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
