...
class CKnife_Zombi_Boomer : public CKnife
{
public:
...
LINK_ENTITY_TO_CLASS(Knife_Zombi_boomer, CKnife_Zombi_Boomer)
...
void CKnife_Zombi_Boomer::Precache(void)
{
	PRECACHE_MODEL("models/v_knife_zombiboomer.mdl");
	PRECACHE_MODEL("models/v_knife_zombiboomer_st_host.mdl");
	...
	PRECACHE_SOUND("zb3/boomer_stab.wav");

	m_usKnife = PRECACHE_EVENT(1, "events/knife.sc");
}

int CKnife_Zombi_Boomer::GetItemInfo(ItemInfo *p)
{
...
BOOL CKnife_Zombi_Boomer::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	...
	{
		return DefaultDeploy("models/v_knife_zombiboomer_st_host.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
	else
	{	
		return DefaultDeploy("models/v_knife_zombiboomer.mdl", "", KNIFE_DRAW, "knife", UseDecrement() != FALSE);
	}
}

void CKnife_Zombi_Boomer::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CKnife_Zombi_Boomer::SecondaryAttack(void)
{
	Stab(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 0.35;
}

void CKnife_Zombi_Boomer::Holster(int skiplocal)
{
	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_ITEM, "zb3/boomer_draw.wav", VOL_NORM, ATTN_NORM);
}

void CKnife_Zombi_Boomer::WeaponIdle(void)
{
	ResetEmptySound();
...
int CKnife_Zombi_Boomer::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		SetThink(&CKnife_Zombi_Boomer::Smack);
...
int CKnife_Zombi_Boomer::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
	...
	else
	{
		...
		if (pEntity)
		{
			if (pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE)
			{
				EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "zb3/boomer_stab.wav", VOL_NORM, ATTN_NORM);
				m_pPlayer->m_iWeaponVolume = KNIFE_BODYHIT_VOLUME;
