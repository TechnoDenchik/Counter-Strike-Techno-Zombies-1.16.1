...
{
	PRECACHE_MODEL("models/v_shelteraxe.mdl");
...
{
	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/tomahawk_draw.wav", VOL_NORM , ATTN_NORM);
...
{
	Swing(TRUE);
	pev->nextthink = UTIL_WeaponTimeBase() + 2.95;
}
...
int CShelter_axe::Swing(int fFirst)
{
	...
	if (tr.flFraction >= 1)
	{
		if (fFirst)
		{
			...
			if (RANDOM_LONG(0, 1))
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/tomahawk_slash1.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
			else
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/tomahawk_slash2.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
	...
	else
	{
		...
		{
			float flDamage = 15;
			if (m_flNextPrimaryAttack + 0.7 < UTIL_WeaponTimeBase())
				flDamage = 20;
		...
		if (fHitWorld)
		{
			...
			EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/tomahawk_wall.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
		}
#endif
