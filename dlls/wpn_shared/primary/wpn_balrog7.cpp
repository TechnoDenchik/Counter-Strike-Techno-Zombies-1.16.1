...
#include "gamemode/mods.h"
	...
	protected:
		...
		void Callback()
		{
			...
			this->pev->origin[2] += float(30.0f / (2.0f / 0.01f));
			pev->scale += (0.75 - 0.1) / (2.0f / 0.01f);
			pev->renderamt -= int(200 / (2.0f / 0.01f));
			if (flife + 0.75f < gpGlobals->time)
				Remove();
		}
	};
	...
	void CBalrog7::PrimaryAttack(void)
	{
		if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
			Balrog7Fire(0.05 + (0.2) * m_flAccuracy, 0.1145f, FALSE);
		else if (!FBitSet(m_pPlayer->pev->flags, FL_DUCKING))
			Balrog7Fire(0.01 + (0.02) * m_flAccuracy, 0.1145f, FALSE);
		else if (m_pPlayer->pev->velocity.Length2D() > 195)
			Balrog7Fire(0.035 + (0.03) * m_flAccuracy, 0.1145f, FALSE);
		else if (m_pPlayer->pev->fov == 65)
			Balrog7Fire((0.01) * m_flAccuracy + 0.02, 0.135f, FALSE);
		else
			Balrog7Fire(0.021, 0.1145f, FALSE);
	}
	...
	void CBalrog7::Balrog7Fire(float flSpread, duration_t flCycleTime, BOOL fUseAutoAim)
	{
		...
		if (iShootTime > 10)
		{
			...
			RadiusDamage(vecShootPosition, BalrogDamage());
	...
	void CBalrog7::Reload(void)
	{
		...
		if (DefaultReload(BALROG7_MAX_CLIP, BALROG7_RELOAD, 3.8f))
		{
#ifndef CLIENT_DLL
