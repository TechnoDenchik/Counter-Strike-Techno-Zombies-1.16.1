...
void CEthereal::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_ethereal");
...
void CEthereal::Precache(void)
{
	...
	m_usFireAK47 = PRECACHE_EVENT(1, "events/ethereal.sc");
}
...
BOOL CEthereal::Deploy(void)
{
	m_flAccuracy = 0.2;
	m_iShotsFired = 0;
	iShellOn = 1;
	return DefaultDeploy("models/v_ethereal.mdl", "models/w_ethereal.mdl", ETHEREAL_DRAW, "ak47", UseDecrement() != FALSE);
}

void CEthereal::PrimaryAttack(void)
{
	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
		ETHEREALFire(0.04 + (0.4) * m_flAccuracy, 0.0955, FALSE);
	else if (m_pPlayer->pev->velocity.Length2D() > 140)
		ETHEREALFire(0.04 + (0.07) * m_flAccuracy, 0.0955, FALSE);
	else
		ETHEREALFire((0.0275), 0.0955, FALSE);
}

void CEthereal::ETHEREALFire(float flSpread, float flCycleTime, BOOL fUseAutoAim)
{
	m_bDelayFire = true;
	m_iShotsFired++;
	...
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/ethereal-1.wav", VOL_NORM, ATTN_NORM);

	switch (RANDOM_LONG(1,3))
	{
	case 1:
		SendWeaponAnim(ETHEREAL_SHOOT1, UseDecrement() != FALSE);
		break;
	case 2:
		SendWeaponAnim(ETHEREAL_SHOOT2, UseDecrement() != FALSE);
		break;
	case 3:
		SendWeaponAnim(ETHEREAL_SHOOT3, UseDecrement() != FALSE);
		break;
	}
...
void CEthereal::Reload(void)
{
	...
	if (DefaultReload(AK47_MAX_CLIP, ETHEREAL_RELOAD, 2.45))
	{
#ifndef CLIENT_DLL
...
void CEthereal::WeaponIdle(void)
{
	...
	SendWeaponAnim(ETHEREAL_IDLE1, UseDecrement() != FALSE);
}
