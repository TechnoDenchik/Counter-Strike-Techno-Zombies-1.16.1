...
void CStg44::PrimaryAttack(void)
{
	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
		Stg44Fire(0.04 + (0.4) * m_flAccuracy, 0.0955f, FALSE);
	else if (m_pPlayer->pev->velocity.Length2D() > 140)
		Stg44Fire(0.04 + (0.07) * m_flAccuracy, 0.0955f, FALSE);
	else
		Stg44Fire((0.0275), 0.0955f, FALSE);
}
...
void CStg44::Reload(void)
{
	if (m_pPlayer->ammo_762nato <= 0)
		return;
	if (DefaultReload(AK47_MAX_CLIP, STG44_RELOAD, 2.45f))
	{
#ifndef CLIENT_DLL
