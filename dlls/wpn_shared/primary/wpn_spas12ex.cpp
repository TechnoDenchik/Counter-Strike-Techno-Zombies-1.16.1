...
enum spas12ex_e
{
	...
	SPAS12EX_RELOAD,
	SPAS12EX_PUMP,
	SPAS12EX_START_RELOAD,
	SPAS12EX_DRAW
};

LINK_ENTITY_TO_CLASS(weapon_spas12ex, CSpas12ex)

void CSpas12ex::Spawn(void)
{
	Precache();
	m_iId = WEAPON_XM1014;
	SET_MODEL(ENT(pev), "models/w_spas12ex.mdl");
...
void CSpas12ex::Precache(void)
{
	...
	PRECACHE_SOUND("weapons/spas12excraft_boltpull.wav");
	PRECACHE_SOUND("weapons/spas12excraft_clipin.wav");
	PRECACHE_SOUND("weapons/spas12excraft_clipout.wav");
	PRECACHE_SOUND("weapons/spas12excraft_draw.wav");
	PRECACHE_SOUND("weapons/spas12excraft-1.wav");

	m_usFireXM1014 = PRECACHE_EVENT(1, "events/spas12ex.sc");
}

int CSpas12ex::GetItemInfo(ItemInfo *p)
{
...
BOOL CSpas12ex::Deploy(void)
{
	return DefaultDeploy("models/v_spas12ex.mdl", "models/p_spas12ex.mdl", SPAS12EX_DRAW, "m249", UseDecrement() != FALSE);
}

void CSpas12ex::PrimaryAttack(void)
{
	...
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/spas12ex-1.wav", VOL_NORM, ATTN_NORM);

	switch (RANDOM_LONG(1, 2))
	{
	case 1:
		SendWeaponAnim(SPAS12EX_FIRE1, UseDecrement() != FALSE);
		break;
	case 2:
		SendWeaponAnim(SPAS12EX_FIRE2, UseDecrement() != FALSE);
		break;
	}
...
void CSpas12ex::Reload(void)
{
...
#endif
		SendWeaponAnim(SPAS12EX_START_RELOAD, UseDecrement() != FALSE);
	...
	else if (m_fInSpecialReload == 1)
	{
		...
		SendWeaponAnim(SPAS12EX_RELOAD, UseDecrement());
...
void CSpas12ex::WeaponIdle(void)
{
	ResetEmptySound();
	...
	if (m_flTimeWeaponIdle < UTIL_WeaponTimeBase())
	{
		...
		else if (m_fInSpecialReload != 0)
		{
			...
			{
				SendWeaponAnim(SPAS12EX_PUMP, UseDecrement() != FALSE);
			...
			SendWeaponAnim(SPAS12EX_IDLE, UseDecrement() != FALSE);
	}
}
