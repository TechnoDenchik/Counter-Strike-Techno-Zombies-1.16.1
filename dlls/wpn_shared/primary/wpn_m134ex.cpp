...
void CM134ex::PrimaryAttack(void)
{
	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
		M134exFire(0.065 + (0.75) * m_flAccuracy, 0.06155f, FALSE);
	else if (m_pPlayer->pev->velocity.Length2D() > 140)
		M134exFire(0.045 + (0.095) * m_flAccuracy, 0.06155f, FALSE);
	else
		M134exFire((0.04) * m_flAccuracy, 0.06155f, FALSE);
}
...
void CM134ex::ItemPostFrame()
{
	if (m_pPlayer->pev->button & IN_ATTACK)
	{
		...
		{
			if (m_flNextPrimaryAttack <= 0.0f && m_iClip)
			{
				...
				m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 1.06f;
				m_iM134State = WPNSTATE_M134_SPIN_UP;
				m_pPlayer->pev->button &= ~IN_ATTACK;
			}
		}

		if (m_iM134State == WPNSTATE_M134_SPIN_UP && m_flNextPrimaryAttack <= 0.0f)
		{
			m_pPlayer->pev->maxspeed = M134_FIRE_MAX_SPEED;
	...
	else if(!(m_pPlayer->pev->button & IN_ATTACK2))
	{
		...
		if (m_iM134State == WPNSTATE_M134_SPIN_DOWN && m_flNextPrimaryAttack <= 0.0f)
		{
			m_pPlayer->pev->maxspeed = M134_MAX_SPEED;
			m_iM134State = WPNSTATE_M134_IDLE;
			m_pPlayer->pev->button &= ~IN_ATTACK;
	...
	if (m_pPlayer->pev->button & IN_ATTACK2 && m_iClip)
	{
		if (m_iM134State == WPNSTATE_M134_IDLE)
		{
			if (m_flNextPrimaryAttack <= 0.0f)
			{
				...
				m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 1.8f;
				m_iM134State = WPNSTATE_M134_SPIN_UP;
				m_pPlayer->pev->button &= ~IN_ATTACK2;
			}
		}

		else if ((m_iM134State == WPNSTATE_M134_SPINNING || m_iM134State == WPNSTATE_M134_SPIN_UP) && m_flNextPrimaryAttack <= 0.0f)
		{
...
void CM134ex::Reload(void)
{
	...
	if (DefaultReload(M249_MAX_CLIP, M134_RELOAD, 5.0f))
	{
#ifndef CLIENT_DLL
