...
#include "gamemode/mods.h"
		...
		public:
			void Spawn() override
			{
				...
				m_flExplodeTime = gpGlobals->time + 30.0f;
			...
			void OnThink()
			{
				...
				{
					this->pev->frame = (float)(this->pev->framerate * gpGlobals->frametime / 1.0f) + this->pev->frame;
					if (pev->frame > m_flMaxFrames)
					{
						pev->frame = fmod(pev->frame, m_flMaxFrames);
					}
					pev->nextthink = gpGlobals->time + 0.0099999998f;
...
void CGatlingex::SecondaryAttack(void)
{
	...
	m_flNextPrimaryAttack = m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 2.0f;
}
...
void CGatlingex::PrimaryAttack(void)
{
	...
	m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.23f;
	m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.23f;
...
void CGatlingex::Reload(void)
{
	...
	if (DefaultReload(GATLING_MAX_CLIP, GATLINGEX_RELOAD, 4.55f))
	{
#ifndef CLIENT_DLL
