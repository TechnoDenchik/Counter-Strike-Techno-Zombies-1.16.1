...
#include "gamemode/mods.h"
	...
	{
		if (m_pPlayer->pev->button & IN_ATTACK2 && m_pPlayer->m_flNextAttack <= 0.0f && !m_fInReload)
		{
			m_pPlayer->pev->button &= ~IN_ATTACK2;
			SecondaryAttack();
		}
		if (IsSignal() && (gpGlobals->time > m_flLastFire + 9.0f))
		{
			m_iShotsCount = 0;
			m_flTimeWeaponIdle = 0.0f;
		}
		else if (m_iWeaponState & WPNSTATE_M4A1_SILENCED && (gpGlobals->time > m_flLastFire + 15.0f))
		{
			m_iShotsCount = 0;
	...
	void CJanus7xmas::PrimaryAttack(void)
	{
		...
		else
		{
			if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
				Janus7xmasFire(0.045 + (0.5) * m_flAccuracy, 0.095f, FALSE);
			else if (m_pPlayer->pev->velocity.Length2D() > 140)
				Janus7xmasFire(0.045 + (0.095) * m_flAccuracy, 0.095f, FALSE);
			else
				Janus7xmasFire((0.03) * m_flAccuracy, 0.095f, FALSE);
...
*/
	...
	void CJanus7xmas::Reload(void)
	{
		...
		if (IsSignal())
		{
			if (DefaultReload(WEAPON_CLIP, ANIM_RELOAD_SIGNAL, 4.8f))
			{
#ifndef CLIENT_DLL
		...
		{
			if (DefaultReload(WEAPON_CLIP, ANIM_RELOAD, 4.8f))
			{
#ifndef CLIENT_DLL
				m_pPlayer->SetAnimation(PLAYER_RELOAD);
#endif
