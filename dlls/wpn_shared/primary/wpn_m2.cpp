...
#include "gamemode/mods.h"
	...
	void CM2::SecondaryAttack(void)
	{
		if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
		{
			...
			m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 3.8f;
			m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 3.8f;
			m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 4.0f;
			fTime = gpGlobals->time;
			SetThink(&CM2::SwitchThink);
			pev->nextthink = gpGlobals->time + 3.8f;
		}
		else
		{
			...
			m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 4.7f;
			m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 4.7f;
			m_pPlayer->ResetMaxSpeed();
		}
	}
	...
	void CM2::PrimaryAttack(void)
	{
		if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
		{
			M2Fire((0.02) * m_flAccuracy, 0.1f, FALSE);
		}
		else
		{
			if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
				M2Fire(0.1 + (0.3) * m_flAccuracy, 0.3f, FALSE);
			else if (m_pPlayer->pev->velocity.Length2D() > 140)
				M2Fire(0.06 + (0.095) * m_flAccuracy, 0.3f, FALSE);
			else if (m_pPlayer->pev->velocity.Length2D() <= 140)
				M2Fire(0.1 + (0.06) * m_flAccuracy, 0.3f, FALSE);
			else
				M2Fire((0.06) * m_flAccuracy, 0.3f, FALSE);
		}
	}
...
*/
	...
	void CM2::Reload(void)
	{
		...
		{
			iAnim = M2_RELOADB;
			if (DefaultReload(M2_DEFAULT_GIVE, iAnim, 5.73f))
			{
#ifndef CLIENT_DLL
		...
		{
			iAnim = M2_RELOADA;
			if (DefaultReload(M2_DEFAULT_GIVE, iAnim, 6.1f))
			{
#ifndef CLIENT_DLL
				m_pPlayer->SetAnimation(PLAYER_RELOAD);
#endif
