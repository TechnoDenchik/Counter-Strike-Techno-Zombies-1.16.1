/* =================================================================================== *
			 * =================== TechnoSoftware =================== *
 * =================================================================================== */

#include "extdll.h"
#include "util.h"
#include "cbase.h"
#include "player.h"
#include "weapons.h"
#include "wpn_failnaught.h"

#include <vector>
#include <array>

#ifndef CLIENT_DLL

#include "effects.h"
#include "customentity.h"
#include "monsters.h"
#include "gamemode/mods.h"

	Vector Get_Velocity_Angle2(CBaseEntity *pEntity, Vector Output)
	{
		pEntity->pev->velocity;
		Output = UTIL_VecToAngles(pEntity->pev->velocity);
		if (Output[0] > 90.0)
			Output[0] = -(360.0 - Output[0]);
		return Output;
	}

	Vector Stock_Get_Velocity_Angle2(Vector In, Vector Output)
	{
		Output = UTIL_VecToAngles(In);
		if (Output[0] > 90.0)
			Output[0] = -(360.0 - Output[0]);
		return Output;
	}
#endif

#ifndef CLIENT_DLL
	class CHuntBowArrow : public CBaseEntity
	{
	public:
		void Spawn() override
		{
			Precache();

			SET_MODEL(this->edict(), "models/huntbow_arrow.mdl");
			SetThink(&CHuntBowArrow::OnThink);
			SetTouch(&CHuntBowArrow::OnTouch);
			pev->mins = { -0.1, -0.1, -0.1 };
			pev->maxs = { 0.1, 0.1, 0.1 };
			pev->rendermode = kRenderTransAdd; // 5;
			pev->renderfx = kRenderFxNone;
			pev->renderamt = 230.0;		
			pev->nextthink = gpGlobals->time + 0.15f;
			pev->framerate = 1;
			pev->solid = SOLID_BBOX; // 2
			pev->movetype = MOVETYPE_FLY; // 9

			pev->classname = MAKE_STRING("d_arrow");
		}

		void EXPORT OnTouch(CBaseEntity *pOther)
		{
			if (pev->owner == pOther->edict())
				return;
			pev->solid = SOLID_NOT;

			int DECAL_SHOT[5];
			DECAL_SHOT[0] = DECAL_INDEX("{shot1");
			DECAL_SHOT[1] = DECAL_INDEX("{shot2");
			DECAL_SHOT[2] = DECAL_INDEX("{shot3");
			DECAL_SHOT[3] = DECAL_INDEX("{shot4");
			DECAL_SHOT[4] = DECAL_INDEX("{shot5");

			Vector vecStart,vecAngle,vecForw,vecEnd;
			vecStart = pev->origin;
			vecAngle = Get_Velocity_Angle2(this, vecAngle);

			UTIL_MakeVectors(vecAngle);		
			vecForw = gpGlobals->v_forward * 20.0;
			vecEnd = vecStart + vecForw;
			vecForw = gpGlobals->v_forward * -0.5;
			vecStart + vecForw;

			CBaseEntity *pAttacker = CBaseEntity::Instance(pev->owner);
			CBasePlayer *pAttackePlayer = nullptr;

			if (pAttacker && pAttacker->IsPlayer())
				pAttackePlayer = static_cast<CBasePlayer *>(pAttacker);
		
			TraceResult tr;
			UTIL_TraceLine(vecStart, vecEnd, dont_ignore_monsters, ENT(pAttackePlayer->pev), &tr);
			
			if (pAttackePlayer &&
				pOther->pev->takedamage != DAMAGE_NO &&
				pOther->IsAlive()
				)
			{
				Vector vecDirection = (pev->velocity).Normalize();

				ClearMultiDamage();
				pOther->TraceAttack(pAttackePlayer->pev, m_ArrowDamage, vecDirection, &tr, DMG_BULLET | DMG_NEVERGIB);
				ApplyMultiDamage(pAttackePlayer->pev, pAttackePlayer->pev);

				/*CBasePlayer *pVictim = dynamic_cast<CBasePlayer *>(pOther);
				if (pVictim->m_bIsZombie) // Zombie Knockback...
				{
					ApplyKnockbackData(pVictim, vecDirection, { 1500, 500, 700, 600, 0.4f });
					
				}*/
				pev->renderamt = 0.0;
				EMIT_SOUND_DYN(this->edict(), CHAN_AUTO, "weapons/failnaught-1.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
			}

			SetThink(&CBaseEntity::SUB_StartFadeOut);
			pev->nextthink = gpGlobals->time + 6.0f;
			pev->rendermode = kRenderNormal;
			pev->renderamt = 255.0;

			if (pOther->IsBSPModel())
			{
				MESSAGE_BEGIN(MSG_BROADCAST, SVC_TEMPENTITY, pev->origin);
				WRITE_BYTE(TE_GUNSHOTDECAL);
				WRITE_COORD(pev->origin.x);
				WRITE_COORD(pev->origin.y);
				WRITE_COORD(pev->origin.z);
				WRITE_SHORT(0);
				WRITE_BYTE(DECAL_SHOT[RANDOM_LONG(0, 4)]);
				MESSAGE_END();

				EMIT_SOUND_DYN(this->edict(), CHAN_AUTO, "weapons/failnaught_shoot1_empty.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
				pev->animtime = gpGlobals->time;
				pev->framerate = 1.0;
				pev->sequence = 0;
				SetThink(&CBaseEntity::SUB_StartFadeOut);
				pev->nextthink = gpGlobals->time + 3.5f;
				pev->renderamt = 255.0;
				pev->movetype = MOVETYPE_NONE;
				return;
			}
			else
				pev->flags |= FL_KILLME;

			pev->movetype = MOVETYPE_NONE;
		}

		void Precache() override
		{
			m_iModelLight1 = PRECACHE_MODEL("sprites/ef_huntbow_trail.spr");
			PRECACHE_MODEL("models/huntbow_arrow.mdl");
		}

		void OnThink()
		{
			if (pev->flags & FL_KILLME)
				return;
			MESSAGE_BEGIN(MSG_BROADCAST, SVC_TEMPENTITY);
			WRITE_BYTE(TE_BEAMFOLLOW);
			WRITE_SHORT(this->entindex());  // short (entity:attachment to follow)
			WRITE_SHORT(m_iModelLight1);// short (sprite index)
			WRITE_BYTE(20);
			WRITE_BYTE(10);
			WRITE_BYTE(244);
			WRITE_BYTE(244);
			WRITE_BYTE(244);
			WRITE_BYTE(255);
			MESSAGE_END();

			MESSAGE_BEGIN(MSG_BROADCAST, SVC_TEMPENTITY);
			WRITE_BYTE(TE_BEAMFOLLOW);
			WRITE_SHORT(this->entindex());  // short (entity:attachment to follow)
			WRITE_SHORT(m_iModelLight1);// short (sprite index)
			WRITE_BYTE(20);
			WRITE_BYTE(10);
			WRITE_BYTE(244);
			WRITE_BYTE(244);
			WRITE_BYTE(244);
			WRITE_BYTE(255);
			MESSAGE_END();

			pev->nextthink = gpGlobals->time + 10.0f;
		}

		void Init(Vector vecVelocity, float flDamage, TeamName iTeam, int iMode, float flGravity)
		{
			pev->gravity = flGravity;
			m_iMode = iMode;
			m_ArrowDamage = flDamage;
			pev->velocity = std::move(vecVelocity);
			m_iTeam = iTeam;
		}

		int m_iModelLight1;
		int m_iMode;
		float m_ArrowDamage;
		TeamName m_iTeam;

	protected:
		void Remove()
		{
			SetTouch(nullptr);
			SetThink(nullptr);
			pev->effects |= EF_NODRAW; // 0x80u
			return UTIL_Remove(this);
		}

	};

	LINK_ENTITY_TO_CLASS(huntbow_arrow, CHuntBowArrow);
#endif

enum huntbow_e
{
	ANIM_IDLE,
	ANIM_IDLE_EMPTY,
	ANIM_SHOOT1,
	ANIM_SHOOT1_EMPTY,
	ANIM_DRAW,
	ANIM_DRAW_EMPTY,
	ANIM_CHARGE_START1,
	ANIM_CHARGE_FINISH1,
	ANIM_CHARGE_IDLE1,
	ANIM_CHARGE_IDLE2,
	ANIM_CHARGE_SHOOT1,
	ANIM_CHARGE_SHOOT1_EMPTY,
	ANIM_CHARGE_SHOOT2,
	ANIM_CHARGE_SHOOT2_EMPTY
};

LINK_ENTITY_TO_CLASS(weapon_huntbow, CFailNaught)

void CFailNaught::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_huntbow");

	Precache();
	m_iId = WEAPON_M249;
	SET_MODEL(ENT(pev), "models/w_huntbow.mdl");
	m_flAccuracy = 0.2;
	m_iShotsFired = 0;

	m_iClip = -1;
	m_iDefaultAmmo = 60;

	FallInit();
}

void CFailNaught::Precache(void)
{
	PRECACHE_MODEL("models/v_huntbow.mdl");
	PRECACHE_MODEL("models/w_huntbow.mdl");
	PRECACHE_MODEL("models/p_huntbow.mdl");

	PRECACHE_SOUND("weapons/xbow_hit1.wav");
	PRECACHE_SOUND("weapons/xbow_hitbod1.wav");

	PRECACHE_SOUND("weapons/failnaught_charge_loop_fx.wav");
	PRECACHE_SOUND("weapons/failnaught_charge_shoot.wav");
	PRECACHE_SOUND("weapons/failnaught_charge_shoot1.wav");
	PRECACHE_SOUND("weapons/failnaught_charge_shoot2.wav");
	PRECACHE_SOUND("weapons/failnaught_charge_start_fx.wav");
	PRECACHE_SOUND("weapons/failnaught_charge_start1.wav");
	PRECACHE_SOUND("weapons/failnaught_draw.wav");
	PRECACHE_SOUND("weapons/failnaught_draw_empty.wav");
	PRECACHE_SOUND("weapons/failnaught_loop_fx.wav");
	PRECACHE_SOUND("weapons/failnaught_shoot1.wav");
	PRECACHE_SOUND("weapons/failnaught_shoot1_empty.wav");
	PRECACHE_SOUND("weapons/failnaught-1.wav");
	PRECACHE_SOUND("weapons/failnaught-2.wav");
	PRECACHE_SOUND("weapons/failnaught-2_exp.wav");

	m_usFireHuntBow = PRECACHE_EVENT(1, "events/huntbow.sc");
}

int CFailNaught::GetItemInfo(ItemInfo *p)
{
	p->pszName = STRING(pev->classname);
	p->pszAmmo1 = "bowarrow";
	p->iMaxAmmo1 = 60;
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->pszAmmo3 = NULL;
	p->iMaxAmmo3 = -1;
	p->pszAmmoGrenade = NULL;
	p->iMaxAmmoGrenade = -1;
	p->iMaxClip = -1;
	p->iSlot = 0;
	p->iPosition = 4;
	p->iId = m_iId = WEAPON_M249;
	p->iFlags = 0;
	p->iWeight = M249_WEIGHT;

	return 1;
}

BOOL CFailNaught::Deploy(void)
{
	m_flAccuracy = 0.2;
	m_iShotsFired = 0;
	iShellOn = 1;
	pev->iuser1 = 0;
	fFinishTime = 1;

	return DefaultDeploy("models/v_huntbow.mdl", "models/p_huntbow.mdl", m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] ? ANIM_DRAW : ANIM_DRAW_EMPTY, "m249", UseDecrement() != FALSE);
}

int CFailNaught::ExtractAmmo(CBasePlayerWeapon *pWeapon)
{
	if (m_iDefaultAmmo)
	{
		m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] = m_iDefaultAmmo;
		m_iClip = WEAPON_NOCLIP;
		m_iDefaultAmmo = 0;
		return TRUE;
	}
	return CBasePlayerWeapon::ExtractAmmo(pWeapon);
}

void CFailNaught::PrimaryAttack(void)
{
	if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] <= 0)
	{
		return;
	}
	m_iShotsFired++;
	m_pPlayer->pev->punchangle.x -= 2.0;
	m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType]--;
#ifndef CLIENT_DLL
	m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
	SendWeaponAnim(m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] ? ANIM_SHOOT1 : ANIM_SHOOT1_EMPTY, UseDecrement() != FALSE);

	m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.425f;
	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.925f;

	PLAYBACK_EVENT_FULL(FEV_GLOBAL, m_pPlayer->edict(), m_usFireHuntBow, 0, m_pPlayer->pev->origin, (float *)&g_vecZero, 0.0, 0.0, (1 << 1), 0, FALSE, TRUE);

#ifndef CLIENT_DLL
	if (!m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] <= 0)
		m_pPlayer->SetSuitUpdate("!HEV_AMO0", FALSE, 0);
#endif
	m_iMode = 0;
#ifndef CLIENT_DLL
	Vector vecArrowAngle;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle + m_pPlayer->pev->punchangle);
	//Vector vecSrcB = m_pPlayer->GetGunPosition() + gpGlobals->v_forward * 10 + -gpGlobals->v_right * (-5);
	CHuntBowArrow*pEnt = static_cast<CHuntBowArrow*>(CBaseEntity::Create("huntbow_arrow", m_pPlayer->GetGunPosition(), Stock_Get_Velocity_Angle2(gpGlobals->v_forward * 2000, vecArrowAngle), ENT(m_pPlayer->pev)));
	if (pEnt)
	{
		pEnt->Init(gpGlobals->v_forward * 2000, GetPrimaryAttackDamage(), m_pPlayer->m_iTeam, m_iMode, 0.15);
	}
#endif

	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
		KickBack(1.0, 1.0, 0.5, 0.5, 5.0, 5.0, 5.0);
	else if (m_pPlayer->pev->velocity.Length2D() > 0)
		KickBack(0.3, 0.3, 0.07, 0.07, 2.0, 2.0, 4.0);
	else if (FBitSet(m_pPlayer->pev->flags, FL_DUCKING))
		KickBack(0.2, 0.2, 0.01, 0.01, 0.5, 0.5, 4.0);
	else
		KickBack(0.2, 0.2, 0.01, 0.01, 0.7, 0.7, 3.0);

	pev->iuser1 = 0;
	fFinishTime = 1;
}

void CFailNaught::SecondaryAttack(void)
{
	if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] <= 0)
	{
		return;
	}
}

void CFailNaught::ItemPostFrame(void)
{
	if (m_flNextPrimaryAttack > 0.0f)
		return CBasePlayerWeapon::ItemPostFrame();
	if (!m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
		return CBasePlayerWeapon::ItemPostFrame();

	if (m_pPlayer->pev->button & IN_ATTACK && !(m_pPlayer->pev->button & IN_ATTACK2) && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		PrimaryAttack();
		m_pPlayer->pev->button &= ~IN_ATTACK;
		return CBasePlayerWeapon::ItemPostFrame();
	}
	else if (m_pPlayer->pev->button & IN_ATTACK2 && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		if (!pev->iuser1)
		{
			pev->iuser1 = 1;
			fFinishTime = gpGlobals->time + 1.7f;
			SendWeaponAnim(ANIM_CHARGE_START1, UseDecrement() != FALSE);
			EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/failnaught_charge_start_fx.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
			m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.425f + 0.15f;
			m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.425f + 0.15f;
			m_pPlayer->pev->maxspeed = 185;
		}
		else if (gpGlobals->time > fFinishTime && pev->iuser1 != 2)
		{
			pev->iuser1 = 2;
			EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/failnaught_charge_loop_fx.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
			SendWeaponAnim(ANIM_CHARGE_FINISH1, UseDecrement() != FALSE);
			m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.2f;
			m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.43f;
		}
		m_pPlayer->pev->button &= ~IN_ATTACK2;
	}
	else if (!(m_pPlayer->pev->button & IN_RELOAD))
	{
		switch (pev->iuser1)
		{
		case 1:
		{
			m_pPlayer->pev->maxspeed = HUNTBOW_MAX_SPEED;
			m_iShotsFired++;
#ifndef CLIENT_DLL
			m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
			m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType]--;
			SendWeaponAnim(m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] ? ANIM_CHARGE_SHOOT1 : ANIM_CHARGE_SHOOT1_EMPTY, UseDecrement() != FALSE);
			if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
			{
				m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 1.28f;
				m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 1.36f;
			}
			else
			{
				m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.7f;
				m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.7f;
			}
			EMIT_SOUND_DYN(this->edict(), CHAN_AUTO, "weapons/failnaught-2.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
			m_iMode = 0;
#ifndef CLIENT_DLL
			Vector vecArrowAngle;
			UTIL_MakeVectors(m_pPlayer->pev->v_angle + m_pPlayer->pev->punchangle);
			//Vector vecSrcB = m_pPlayer->GetGunPosition() + gpGlobals->v_forward * 10 + -gpGlobals->v_right * (-5);
			CHuntBowArrow*pEnt = static_cast<CHuntBowArrow*>(CBaseEntity::Create("huntbow_arrow", m_pPlayer->GetGunPosition(), Stock_Get_Velocity_Angle2(gpGlobals->v_forward * 2200, vecArrowAngle), ENT(m_pPlayer->pev)));
			if (pEnt)
			{
				pEnt->Init(gpGlobals->v_forward * 2200, GetPrimaryAttackDamage(), m_pPlayer->m_iTeam, m_iMode, 0.15);
			}
#endif
			pev->punchangle[0] -= 3.0;
			m_pPlayer->pev->button &= ~IN_ATTACK;
			m_pPlayer->pev->button &= ~IN_ATTACK2;
			pev->iuser1 = 0;

			if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
				KickBack(1.0, 1.0, 0.5, 0.5, 5.0, 5.0, 5.0);
			else if (m_pPlayer->pev->velocity.Length2D() > 0)
				KickBack(0.3, 0.3, 0.07, 0.07, 2.0, 2.0, 4.0);
			else if (FBitSet(m_pPlayer->pev->flags, FL_DUCKING))
				KickBack(0.2, 0.2, 0.01, 0.01, 0.5, 0.5, 4.0);
			else
				KickBack(0.2, 0.2, 0.01, 0.01, 0.7, 0.7, 3.0);

			return CBasePlayerWeapon::ItemPostFrame();
		}
		case 2:
		{
			m_pPlayer->pev->maxspeed = HUNTBOW_MAX_SPEED;
			m_iShotsFired++;
#ifndef CLIENT_DLL
			m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
			UTIL_MakeVectors(m_pPlayer->pev->v_angle + m_pPlayer->pev->punchangle);

			m_pPlayer->m_iWeaponVolume = NORMAL_GUN_VOLUME;
			m_pPlayer->m_iWeaponFlash = BRIGHT_GUN_FLASH;

			Vector vecSrc = m_pPlayer->GetGunPosition();
			m_iMode = 1;
			Vector vecDir = m_pPlayer->FireBullets3(vecSrc, gpGlobals->v_forward, 0.0f, 8192, 7, BULLET_PLAYER_556MM, GetSecondaryAttackDamage(), 0.97, m_pPlayer->pev, FALSE, m_pPlayer->random_seed);
			m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType]--;
			PLAYBACK_EVENT_FULL(FEV_GLOBAL, m_pPlayer->edict(), m_usFireHuntBow, 0, (float *)&g_vecZero, (float *)&g_vecZero, vecDir.x, vecDir.y, m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType], 0, FALSE, FALSE);

			SendWeaponAnim(m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] ? ANIM_CHARGE_SHOOT2 : ANIM_CHARGE_SHOOT2_EMPTY, UseDecrement() != FALSE);

			if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
			{
				m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 1.28f;
				m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 1.28f;
			}
			else
			{
				m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.7f;
				m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.7f;
			}
			pev->punchangle[0] -= 5.0;
			pev->iuser1 = 0;

			if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
				KickBack(1.0, 1.0, 0.5, 0.5, 5.0, 5.0, 5.0);
			else if (m_pPlayer->pev->velocity.Length2D() > 0)
				KickBack(0.3, 0.3, 0.07, 0.07, 2.0, 2.0, 4.0);
			else if (FBitSet(m_pPlayer->pev->flags, FL_DUCKING))
				KickBack(0.2, 0.2, 0.01, 0.01, 0.5, 0.5, 4.0);
			else
				KickBack(0.2, 0.2, 0.01, 0.01, 0.7, 0.7, 3.0);

			m_pPlayer->pev->button &= ~IN_ATTACK;
			m_pPlayer->pev->button &= ~IN_ATTACK2;
			return CBasePlayerWeapon::ItemPostFrame();
		}
		}
		
	}
	m_pPlayer->pev->button &= ~IN_ATTACK;
	m_pPlayer->pev->button &= ~IN_ATTACK2;
	return CBasePlayerWeapon::ItemPostFrame();
}

void CFailNaught::Reload(void)
{
	return;
}

void CFailNaught::WeaponIdle(void)
{
	ResetEmptySound();
	m_pPlayer->GetAutoaimVector(AUTOAIM_10DEGREES);

	if (m_flTimeWeaponIdle > UTIL_WeaponTimeBase())
		return;

	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 20;
	SendWeaponAnim(pev->iuser1 >= 1 ? (pev->iuser1 == 2 ? ANIM_CHARGE_IDLE2 : ANIM_CHARGE_IDLE1) : (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] ? ANIM_IDLE : ANIM_IDLE_EMPTY), UseDecrement() != FALSE);
}

float CFailNaught::GetPrimaryAttackDamage() const
{
	float flDamage = 38;
#ifndef CLIENT_DLL
	if (g_pModRunning->DamageTrack() == DT_ZB)
		flDamage = 772;
	else if (g_pModRunning->DamageTrack() == DT_ZBS)
		flDamage = 1772;
#endif
	return flDamage;
}

float CFailNaught::GetSecondaryAttackDamage() const
{
	float flDamage = 152.0f;
#ifndef CLIENT_DLL
	if (g_pModRunning->DamageTrack() == DT_ZB)
		flDamage = 755.0f;
	else if (g_pModRunning->DamageTrack() == DT_ZBS)
		flDamage = 1755.0f;
#endif
	return flDamage;
}