...
BOOL CM95xmas::Deploy(void)
{
	...
	{
		m_flNextPrimaryAttack = m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 1.25f;
...
void CM95xmas::PrimaryAttack(void)
{
	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
		M95xmasFire(1.0, 1.5f, FALSE);
	else if (m_pPlayer->pev->velocity.Length2D() > 120)
		M95xmasFire(0.3, 1.5f, FALSE);
	else if (m_pPlayer->pev->velocity.Length2D() > 10)
		M95xmasFire(0.1, 1.5f, FALSE);
	else if (FBitSet(m_pPlayer->pev->flags, FL_DUCKING))
		M95xmasFire(0.0, 1.5f, FALSE);
	else
		M95xmasFire(0.002, 1.5f, FALSE);
}
