...
#include "wpn_svdex.h"
	...
	public:
		void Spawn() override
		{
			...
			m_flRemoveTime = gpGlobals->time + 5.0f;
			m_flMaxFrames = 300.0;
	...
	void CSVDEX::SecondaryAttack(void)
	{
		...
		m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 1.9f;
	...
	void CSVDEX::PrimaryAttack(void)
	{
		if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
		{
			SVDEXFire2(3.0f, FALSE);
		}
		else
		{
			if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
				SVDEXFire1(0.8f, 0.346f, FALSE);
			else if (m_pPlayer->pev->velocity.Length2D() > 140)
				SVDEXFire1(0.15f, 0.346f, FALSE);
			else if (m_pPlayer->pev->velocity.Length2D() > 10)
				SVDEXFire1(0.1f, 0.346f, FALSE);
			else if (FBitSet(m_pPlayer->pev->flags, FL_DUCKING))
				SVDEXFire1(0.003f, 0.346f, FALSE);
			else
				SVDEXFire1(0.007f, 0.346f, FALSE);
		}
	}
	...
	void CSVDEX::Reload(void)
	{
		...
		if (DefaultReload(20, SVDEX_RELOAD, 3.8f))
		{
#ifndef CLIENT_DLL
