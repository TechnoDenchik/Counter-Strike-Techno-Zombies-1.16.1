...
void CBaseMonster::Killed2(entvars_t* pevAttacker, int iGib)
{
	if (HasMemory(bits_MEMORY_KILLED))
	{
		if (ShouldGibMonster(iGib))
...
int CBaseMonster::TakeDamage2(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage2, int bitsDamageType2)
{
	float flTake;
	Vector vecDir;
	...
	if (!IsAlive())
	{
		return DeadTakeDamage(pevInflictor, pevAttacker, flDamage2, bitsDamageType2);
	}
	...
	// set damage type sustained
	m_bitsDamageType |= bitsDamageType2;
	...
	if (pev->health <= 0.0f)
	{
		...
		if (bitsDamageType2 & DMG_ALWAYSGIB)
			Killed(pevAttacker, GIB_ALWAYS);

		else if (bitsDamageType2 & DMG_NEVERGIB)
			Killed(pevAttacker, GIB_NEVER);
		else
			Killed(pevAttacker, GIB_NORMAL);
...
int CBaseMonster::DeadTakeDamage2(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage2, int bitsDamageType2)
{
	// grab the vector of the incoming attack. ( pretend that the inflictor is a little lower than it really is, so the body will tend to fly upward a bit).
	Vector vecDir = Vector(0, 0, 0);
	...
	if (!FNullEnt(pevInflictor) && (pevAttacker->solid != SOLID_TRIGGER))
	{
		pev->velocity = pev->velocity + vecDir * -DamageForce(flDamage2);
	}

#endif

	// kill the corpse if enough damage was done to destroy the corpse and the damage is of a type that is allowed to destroy the corpse.
	if (bitsDamageType2 & DMG_GIB_CORPSE)
	{
		if (pev->health <= flDamage2)
		{
			pev->health = -50;
			Killed(pevAttacker, GIB_ALWAYS);
			return 0;
