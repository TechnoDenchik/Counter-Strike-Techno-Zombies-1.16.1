...
{
public:
	explicit CTextureRef(const char* path, int flags = TF_NEAREST | TF_NOPICMIP | TF_NOMIPMAP | TF_CLAMP) : m_iInternalId(g_iXash ? gRenderAPI.GL_LoadTexture(path, NULL, 0, flags) : 0) {}
...
public:
	~CTextureRef(){ if (g_iXash)gRenderAPI.GL_FreeTexture(m_iInternalId); }
public:
