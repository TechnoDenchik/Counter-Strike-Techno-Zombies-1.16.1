...
void EV_WonderCannonFireEffect(vec3_t vecSrc, vec3_t vecForward, vec3_t vecVelocity, int idx, int iModel)
{
	if (!iModel)
		iModel = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/muzzleflash111.spr");
	TEMPENTITY *ent;
	int i;
	int ran;
	float c, s;
...
void EV_WonderCannonIdleEffect(vec3_t vecSrc, vec3_t vecForward, vec3_t vecVelocity, int idx, int iModel)
{
	if (!iModel)
		iModel = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/muzzleflash112.spr");
	TEMPENTITY* ent;
	int i;
	int ran;
	float c, s;
...
void EV_FireWonderCannon( event_args_t *args )
{
	...
	PLAY_EVENT_SOUND( "weapons/wondercannon-1.wav" );
	
	EV_GetGunPosition( args, vecSrc, origin );
	VectorCopy( forward, vecAiming );

	const struct mstudioevent_s* event;
	const char iModel = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/muzzleflash112.spr");
...
void EV_Fire2WonderCannon(event_args_t* args)
{
	...
	{
		++g_iShotsFired;
		gEngfuncs.pEventAPI->EV_WeaponAnimation(ANIM_IDLE, 2);
		///EV_MuzzleFlash();
		if (!gHUD.cl_righthand->value)
		{
			EV_GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 20.0, -8.0, -10.0, 0);
	...
	if (EV_IsLocal(idx))
		EV_WonderCannonIdleEffect(ent->attachment[7], origin, velocity, idx, 0);
	//else
