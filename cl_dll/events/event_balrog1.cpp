...
void EV_FireBALROG1( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		if( args->bparam1 )
		{
			gEngfuncs.pEventAPI->EV_WeaponAnimation(BALROG_SHOOT1, 2 );
		}
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, forward, vSpread, 8192.0, BULLET_PLAYER_50AE, 2 );
}

void EV_Fire2BALROG1(event_args_t* args)
{
	vec3_t ShellVelocity;
	vec3_t ShellOrigin;
	vec3_t vecSrc;
	...
	if (EV_IsLocal(idx))
	{
		...
		if (args->bparam1)
		{
			gEngfuncs.pEventAPI->EV_WeaponAnimation(BALROG_SHOOT2, 2);
		}
