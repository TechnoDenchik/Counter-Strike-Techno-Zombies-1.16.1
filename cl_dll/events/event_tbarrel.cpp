...
static const char *SOUNDS_NAME = "weapons/qbarrel-1.wav";

void EV_FireTbarrel(event_args_s *args)
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		EV_MuzzleFlash();
		if(args->bparam2)
			gEngfuncs.pEventAPI->EV_WeaponAnimation(ANIM_FIRE2, 2);
		else
			gEngfuncs.pEventAPI->EV_WeaponAnimation(ANIM_FIRE1, 2);
