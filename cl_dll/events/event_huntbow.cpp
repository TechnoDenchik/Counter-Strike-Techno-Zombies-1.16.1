...
void EV_FireHuntBow(event_args_s *args)
{
	int idx, effect;
	vec3_t origin;
	vec3_t angles;
	...
	if (args->bparam2)
	{
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "sound/weapons/failnaught-1", 1.0, ATTN_NORM, 0, PITCH_NORM);
	...
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, args->iparam1 ? "sound/weapons/failnaught_charge_shoot2.wav" : "sound/weapons/failnaught_shoot1_empty.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);

	int iBeamModelIndex = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/ef_huntbow_trail.spr");
