...
void EV_WonderCannonExFireEffect(vec3_t vecSrc, vec3_t vecForward, vec3_t vecVelocity, int idx, int iModel)
{
	if (!iModel)
		iModel = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/muzzleflash255.spr");
	TEMPENTITY *ent;
	int i;
	int ran;
	float c, s;
...
void EV_FireWonderCannonEx( event_args_t *args )
{
