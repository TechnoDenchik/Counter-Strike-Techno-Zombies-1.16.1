...
void EV_FireVoidPistolEx( event_args_t *args )
{
	...
	else
	{
		if (args->bparam2)
		{
			...
			case BLACKHOLE_START:
			{
				...
				struct model_s* pModel = IEngineStudio.Mod_ForName("sprites/ef_voidpistolex_start.spr", false);
				if (pModel)
				{
			...
			{
				//blackhole_projectile spr
				struct model_s* pModel = IEngineStudio.Mod_ForName("sprites/ef_voidpistolex_loop.spr", false);
				if (pModel)
				{
					TEMPENTITY* pEnt = gEngfuncs.pEfxAPI->CL_TempEntAllocHigh(origin, pModel);
					pEnt->entity.curstate.frame = 0;
					pEnt->entity.curstate.framerate = 35.0;
				...
				struct model_s* pModel2 = IEngineStudio.Mod_ForName("sprites/ef_voidpistolex_loop.spr", false);
				if (pModel)
				{
					TEMPENTITY* pEnt = gEngfuncs.pEfxAPI->CL_TempEntAllocHigh(origin, pModel2);
					pEnt->entity.curstate.frame = 0;
					pEnt->entity.curstate.framerate = 35.0;
			...
			case BLACKHOLE_END:
			{
				struct model_s* pModel = IEngineStudio.Mod_ForName("sprites/ef_voidpistolex_end.spr", false);
				if (pModel)
				{
				...
				struct model_s* pModel2 = IEngineStudio.Mod_ForName("sprites/ef_voidpistolex_end.spr", false);
				if (pModel)
				{
					gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/voidpistol_blackhole_exp.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
					TEMPENTITY* pEnt = gEngfuncs.pEfxAPI->CL_TempEntAllocHigh(origin, pModel2);
			...
			{
				//blackhole_projectile spr
				struct model_s* pModel = IEngineStudio.Mod_ForName("sprites/ef_voidpistolex_projectile.spr", false);
				if (pModel)
				{
				...
				struct model_s* pModel2 = IEngineStudio.Mod_ForName("sprites/ef_voidpistolex_projectile.spr", false);
				if (pModel2)
				{
