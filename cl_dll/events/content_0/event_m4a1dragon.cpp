...
void EV_FireM4A1DRAGON( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		if( args->bparam1 )
		{
			sequence = Com_RandomLong(M4A1DRAGON_SHOOT1, M4A1DRAGON_SHOOT3);
		}
		else
		{
			sequence = Com_RandomLong(M4A1DRAGON_UNSIL_SHOOT1, M4A1DRAGON_UNSIL_SHOOT3);
		}
