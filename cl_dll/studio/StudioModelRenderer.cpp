...
void CStudioModelRenderer::StudioSetupBones(void)
{
	...
	for (i = 0; i < m_pStudioHeader->numbones; i++)
	{
		...
		{
			// fix knife
			if (gHUD.cl_righthand && gHUD.cl_righthand->value > 0 && bIsViewModel && IEngineStudio.IsHardware() && g_bHoldingKnife/*g_iCurrentWeapon != WEAPON_KNIFE*/)
			{
...
void CStudioModelRenderer::StudioCalcAttachments(void)
{
	...
	if (m_pStudioHeader->numattachments > 1024)
