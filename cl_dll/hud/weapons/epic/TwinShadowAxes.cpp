...
int CHudTwinAxes::VidInit(void)
{
	...
	ef_dgaxe_smoke.SetSpriteByName("twinsmoke");

	return 1;
}

int CHudTwinAxes::Draw(float time)
{
	if (!m_pCurTexture)
		return 1;

	if (!ef_dgaxe_smoke2)
		ef_dgaxe_smoke2 = LoadSprite("sprites/ef_dgaxe_smoke.spr");
	...
	y2 = ScreenHeight - SPR_Height(ef_dgaxe_smoke2, 0) - gHUD.m_iFontHeight;
	x2 = ScreenWidth / 3 + SPR_Width(ef_dgaxe_smoke2, 0) / 4;
	...
	//m_pCurTexture->Bind();
	rc = ef_dgaxe_smoke.rect;
	SPR_Set(ef_dgaxe_smoke.spr, r, g, b);
	SPR_DrawAdditive(0, x2, y2, &rc);
	//DrawUtils::Draw2DQuadScaled(x - 881 / 3.0, y - 3.5, x + 881 / 3.0, y + 120);
	return 1;
}

void CHudTwinAxes::Setint()
{
	m_pCurTexture = winct;
