...
int CHudFollowIcon::VidInit(void)
{
	...
	if (!m_pTexture_virus)
		m_pTexture_virus = R_LoadTextureUnique("resource/zb5/zh5_virus_icon");
...
int CHudFollowIcon::Draw(float time)
{
	...
	if (gHUD.m_iModRunning == MOD_ZB2 || gHUD.m_iModRunning == MOD_ZB3 || gHUD.m_iModRunning == MOD_ZB5)
	{
		...
		else if(g_PlayerExtraInfo[gHUD.m_Scoreboard.m_iPlayerNum].teamnumber == TEAM_TERRORIST)
		{
			...
			{
				if (g_ZombieBoxInfo[i].dead)
					continue;

				float xyScreen[2];
				if (CalcScreen(g_ZombieBoxInfo[i].origin, xyScreen))
				{
					gEngfuncs.pTriAPI->RenderMode(kRenderTransTexture);
					gEngfuncs.pTriAPI->Color4ub(255, 255, 255, 255);
					m_pTexture_virus->Bind();
					DrawUtils::Draw2DQuadScaled(xyScreen[0] - 18, xyScreen[1] - 18, xyScreen[0] + 19, xyScreen[1] + 19);

					char szBuffer[16];
					sprintf(szBuffer, "[%im]", static_cast<int>((g_ZombieBoxInfo[i].origin - gHUD.m_vecOrigin).Length() / 42.0f));
