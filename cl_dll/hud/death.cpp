	...
	int CHudDeathNotice::MsgFunc_DeathMsg(const char* pszName, int iSize, void* pbuf)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if ((gHUD.m_Scoreboard.m_iTeamAlive_CT) == 1 && g_PlayerExtraInfo[killer].teamnumber == TEAM_TERRORIST)
					{
						if (!g_PlayerExtraInfo[killer].dead && idx == killer)
							NewAlarm().SetAlarm(ALARM_THELAST);
					}
					else if ((gHUD.m_Scoreboard.m_iTeamAlive_T) == 1 && g_PlayerExtraInfo[killer].teamnumber == TEAM_CT)
					{
						if (!g_PlayerExtraInfo[killer].dead && idx == killer)
							NewAlarm().SetAlarm(ALARM_THELAST);
					}
