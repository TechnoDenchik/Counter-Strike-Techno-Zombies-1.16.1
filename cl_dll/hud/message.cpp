...
void CHudMessage::MessageScanNextChar( void )
{
	...
	if ( m_parms.pMessage->effect == 1 && m_parms.charTime != 0 )
	{
		if ( m_parms.x >= 0 && m_parms.y >= 0 && (m_parms.x + gHUD.GetCharWidth( m_parms.text )) <= ScreenWidth )
			DrawUtils::TextMessageDrawChar( m_parms.x, m_parms.y, m_parms.text, m_parms.pMessage->r2, m_parms.pMessage->g2, m_parms.pMessage->b2, 255);
	}
}
	...
	for ( i = 0; i < m_parms.lines; i++ )
	{
		...
		for ( j = 0; j < m_parms.lineLength; j++ )
		{
			...
			if ( m_parms.x >= 0 && m_parms.y >= 0 && next <= ScreenWidth )
				DrawUtils::TextMessageDrawChar( m_parms.x, m_parms.y, m_parms.text, m_parms.r, m_parms.g, m_parms.b, 255);
			m_parms.x = next;
		}
